...
def analisis_FOMUBI(pages_FOMUBI_text):
    ...
    sec_fomubi =  re.findall(r'Sección: (.*?)Manzana:', pages_FOMUBI_text).replace(" ","")
    manz_fomubi =  re.findall(r'Manzana: (.*?)Parcela:', pages_FOMUBI_text).replace(" ","")
    parc_fomubi_0 =  re.findall(r'Parcela: (.*?)Unidad Funcional:', pages_FOMUBI_text).replace(" ","")
