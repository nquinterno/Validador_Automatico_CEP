...
def frente_parcela(lado_parc,manz,tol):
    midpoint = Vec2((lado_parc.dxf.start[0] + lado_parc.dxf.end[0])/2,(lado_parc.dxf.start[1] + lado_parc.dxf.end[1])/2)
    for lo in manz.virtual_entities():
        lo_start = Vec2(lo.dxf.start[0],lo.dxf.start[1])
        lo_end = Vec2(lo.dxf.end[0],lo.dxf.end[1])
        if math.distance_point_line_2d(midpoint,lo_start,lo_end) < tol:
            return True
        else:
            return False
