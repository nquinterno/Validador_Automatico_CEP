...
def analisis_IFTAM(pages_IFTAM_text_0):
    ...
    if (re.search(r'Particularidades: (.*?)Cantidad de plantas: ', pages_IFTAM_text))!=None:
        particularidades =  ((re.search(r'Particularidades: (.*?)Cantidad de plantas: ', pages_IFTAM_text)).group(1)).replace("\n", "")
    
    elif re.search(r'Particularidades\s*(.*?)(\n)', pages_IFTAM_text) != None:

        particularidades = (re.search(r'Particularidades\s*(.*?)(\n)', pages_IFTAM_text).group(1)).replace("Particularidades: ","").replace("\n", "")
    
    else:
        particularidades = "Error: No se pudo detectar"
...
def analisis_IFDOM(pages_IFDOM_text):
    ...
    #expresion_regular = r'Superficie en mts 2: (\d+\,\d+)'
