...
##                lados_parcelas_l.append(parcela.dxf.count)
    ...
    if len(m_manzana)>0:
        for element in m_manzana:
            ...
            validaciones2.loc[9,'Observacion']="ERROR: Verifique haber pegado con coord. originales los elementos del dxf de la manzana en el layer 07-M-MANZANA"
 ...
 #valida que haya la misma cantidad de bloques de parcela insertos en el dxf que la cantidad de parcelas del layer parcelas
    ...
    # Chequear que no haya porqueria en el archivo dxf en un ofset de 50m para afuera de las corrdenadas de la manzana.

##
##    manz = doc.modelspace().query('LWPOLYLINE[layer=="07-M-MANZANA"]')
##
##    vert_m1 = manz.get_points('xy') #para cada mejora obtiene los vertices y los prepara para la función de control
##    vert_m2 = Vec2.list(vert_m1)
##    vert_m3 = list(ezdxf.math.offset_vertices_2d(vert_m2,offset=50, closed=True))
##    vert_m4 = list(ezdxf.math.offset_vertices_2d(vert_m2,offset=-50, closed=True))
##
##    ent_m3 = doc.modelspace().query(f'INSIDE ({vert_m3})')
##    ent_m4 = doc.modelspace().query(f'INSIDE ({vert_m4})')
##
##    if len (ent_m3) > len(ent_m4):
##        ent_m5 = ent_m3
##    else:
##        ent_m5 = ent_m4
##
##    if len(msp)>len(ent_m5):
##
##        validaciones2.loc[32,'Resultado']=-1 # si no hay un -1 en la bandera indica que la validación es correcta.
##        validaciones2.loc[32,'Observacion']="Error: Hay entidades dibujadas en el model por fuera de la manzana que no corresponden a la presentación"
##        validaciones2.loc[32,'Cetegoría']='Formularios'
##
##    else:
##        validaciones2.loc[32,'Resultado']=0 # si no hay un -1 en la bandera indica que la validación es correcta.
##        validaciones2.loc[32,'Observacion']="OK: "
##        validaciones2.loc[32,'Cetegoría']='Formularios'
...
##def convertir_validaciones():
##    global validaciones2
##    global validaciones_dict
##    validaciones_dict = validaciones2.to_dict('records')
##
##    print("validaciones_dict")
##    print(validaciones_dict)
