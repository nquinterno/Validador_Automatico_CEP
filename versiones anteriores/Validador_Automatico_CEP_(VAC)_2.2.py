...
##                lados_parcelas_l.append(parcela.dxf.count)
 ...
 #valida que haya la misma cantidad de bloques de parcela insertos en el dxf que la cantidad de parcelas del layer parcelas
    ...
    for layer in layers_mejoras:
        ...
        for i in range (len(mejora_piso)):
            ...
            if len(mejora_adentro_2)>0:
                ...
                poligono_madre = shapely.Polygon(list(mejora_piso[i].get_points('xy'))) #para cada mejora obtiene los vertices y los prepara para la función de control
                 #genera poligono de mejora hija 1 para iniciar la union de mejoras hijas para luego restar a la madre
                poligono_agujero = shapely.Polygon(list(mejora_adentro_2[0].get_points('xy')))
                for mejora in mejora_adentro_2:
                    #va uniendo los poligonos de las mejoras hijas
                    mejora_poligon = shapely.Polygon(list(mejora.get_points('xy')))
                    poligono_agujero = shapely.union(poligono_agujero,mejora_poligon)
                    if mejora.has_arc:
                        band_mejora_adentro_arc.append("1")
                #resta a la mejora madre el agujero de las mejoras hijas
                poligono_diference = shapely.difference(poligono_madre,poligono_agujero)
                sup_mejora_descont_2 = shapely.area(poligono_diference) #calcula superficie del poligono despues de todas las restas
    ...
    if len(mejoras): #si hay entindades en el layer mejoras comienza la validacion de las mismas sino arroja error que no hay nada en ese layer
        ...
        validaciones2.loc[19,'Observacion']="Error: si se trata de un baldío o CEP demolición debe dibujar un poligono 'descubierto'"
        validaciones2.loc[19,'Cetegoría']='Mejoras'

        validaciones2.loc[20,'Resultado']=-1 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones2.loc[20,'Observacion']="Error: si se trata de un baldío o CEP demolición debe dibujar un poligono 'descubierto'"
        validaciones2.loc[20,'Cetegoría']='Mejoras'

        validaciones2.loc[21,'Resultado']=-1 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones2.loc[21,'Observacion']="Error: si se trata de un baldío o CEP demolición debe dibujar un poligono 'descubierto'"
        validaciones2.loc[21,'Cetegoría']='Mejoras'

        validaciones2.loc[22,'Resultado']=-1 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones2.loc[22,'Observacion']="Error: si se trata de un baldío o CEP demolición debe dibujar un poligono 'descubierto'"
        validaciones2.loc[22,'Cetegoría']='Mejoras'
...
def resumen():
    ...
    if len(pages_IFTAM_text)>0 and len(pages_IFFVN_text)>0 and len(pages_IFDOM_text)>0:
        ...
        nota_cep_demolicion = "El relevamiento de las edificaciones se encuentra exceptuado según Disposición N° DI-2023-66-GCABA-DGROC, la vigencia de la presente constitución de estado parcelario quedará extinta de pleno derecho en el caso de no finalizarse la demolición de las construcciones existentes en el plazo de un (1) año desde la fecha de su constitución"
        
        nota_aph = "La parcela se encuentra catalogada con protección histórica"
        ...
        if cont_3 > 0:
            ...
            if ((nota_cont.lower()).replace(" ","")).replace(".","") in ((notas_0[0]).lower()):
        ...
        if (mensura_3 - sup_tit) >0:
            if (mensura_3 - sup_tit) > (0.05 * mensura_3):
                if ((nota_exce_1.lower().replace(" ","")).replace(".","") in notas_0[0].lower()) or (((nota_exce_2_1.lower().replace(" ","")).replace(".","") in (notas_0[0].lower())) and ((nota_exce_2_2.lower().replace(" ","")).replace(".","") in (notas_0[0].lower()))):
        ...
        if dif_agip =="no":
            if ((nota_agip_nodif.lower().replace(" ","")).replace(".","") in (notas_0[0].lower())) or ((nota_cep_demolicion.lower().replace(" ","")).replace(".","") in (notas_0[0].lower())):
            ...
            if len(agip_supnueva)>0:
                if (nota_agip_dif_mas.lower().replace(" ","")).replace(".","") in (notas_0[0].lower()):
                    validaciones2.loc[43,'Resultado']=0
                ...
                if (nota_agip_dif_menos.lower().replace(" ","")).replace(".","") in (notas_0[0].lower()):
                    validaciones2.loc[43,'Resultado']=0
        ...
        if "-1" in band_cur_parcela:
            ...
            validaciones2.loc[46,'Observacion']="Error: No se puede comprobar APH y afectaciones de la parcela por no poder conectarse con Ciudad 3d, verifique conexión de internet"
        else:
            if  (nota_cep_demolicion.lower().replace(" ","")).replace(".","") in (notas_0[0].lower()):
                if aph != "No":
                    validaciones2.loc[46,'Resultado']=-1
                    validaciones2.loc[46,'Observacion']="Error: Se ha declarado CEP Demolición y la parcela esta afectada a APH"

                else:
                    validaciones2.loc[46,'Resultado']=0
                    validaciones2.loc[46,'Observacion']="Ok: Se ha declarado CEP Demolición y la parcela no esta afectada a APH"

            else:
                validaciones2.loc[46,'Resultado']=0
                validaciones2.loc[46,'Observacion']="Ok: No se ha declarado CEP Demolición"


            if aph != "No":
                if  nota_aph.lower().replace(" ","") in (notas_0[0].lower()):
                    validaciones2.loc[47,'Resultado']=0
                    validaciones2.loc[47,'Observacion']="Ok: Se ha indicado la Nota de Aph en formulario de Mensura"
                else:
                    validaciones2.loc[47,'Resultado']=-1
                    validaciones2.loc[47,'Observacion']="Error: No se ha indicado la Nota de Aph en formulario de Mensura y la parcela se encuentra catalogada"
            else:
                validaciones2.loc[47,'Resultado']=99
                validaciones2.loc[47,'Observacion']="Ok: No corresponde validar nota de Aph"
            ...
            if "Cinturon Digital" in cur_afectaciones:
                if "Cinturon Digital" in particularidades:
                    if "Cinturon Digital".lower().replace(" ","") in (notas_0[0].lower()):
                        ...
                        validaciones2.loc[48,'Observacion']="Ok: Se indicó nota de 'Afectación a Cinturon Digital' en Notas del formulario de Mensura"
                    else:
                        validaciones2.loc[48,'Resultado']=-1
                        validaciones2.loc[48,'Observacion']="Error: No se indicó nota de 'Afectación a Cinturon Digital' en Notas del formulario de Mensura"
                else:
                    validaciones2.loc[48,'Resultado']=-1
                    validaciones2.loc[48,'Observacion']="Error: No se indicó nota de 'Afectación a Cinturon Digital' en Particularidades del formulario de Mensura"
            else:
                validaciones2.loc[48,'Resultado']=99
                validaciones2.loc[48,'Observacion']="Ok: No corresponde validar nota de 'Afectación Cinturon Digital'"
            ...
            if "Afectación por Apertura" in cur_afectaciones:
                if "Afectación por Apertura" in particularidades:
                    if "Afectación por Apertura".lower().replace(" ","") in (notas_0[0].lower()):
                        ...
                        validaciones2.loc[49,'Observacion']="Ok: Se indicó nota de 'Afectación por Apertura' en Notas del formulario de Mensura"
                    else:
                        validaciones2.loc[49,'Resultado']=-1
                        validaciones2.loc[49,'Observacion']="Error: No se indicó nota de 'Afectación por Apertura' en Notas del formulario de Mensura"
                else:
                    validaciones2.loc[49,'Resultado']=-1
                    validaciones2.loc[49,'Observacion']="Error: No se indicó nota de 'Afectación por Apertura' en Particularidades del formulario de Mensura"
            else:
                validaciones2.loc[49,'Resultado']=99
                validaciones2.loc[49,'Observacion']="Ok: No corresponde validar nota 'Afectación por Apertura'"
            ...
            if "Afectación por Ensanche" in cur_afectaciones:
                if "Afectación por Ensanche" in particularidades:
                    if "Afectación por Ensanche".lower().replace(" ","") in (notas_0[0].lower()):
                        ...
                        validaciones2.loc[50,'Observacion']="Ok: Se indicó nota de 'Afectación por Ensanche' en Notas del formulario de Mensura"
                    else:
                        validaciones2.loc[50,'Resultado']=-1
                        validaciones2.loc[50,'Observacion']="Error: No se indicó nota de 'Afectación por Ensanche' en Notas del formulario de Mensura"
                else:
                    validaciones2.loc[50,'Resultado']=-1
                    validaciones2.loc[50,'Observacion']="Error: No se indicó nota de 'Afectación por Ensanche' en Particularidades del formulario de Mensura"
            else:
                validaciones2.loc[50,'Resultado']=99
                validaciones2.loc[50,'Observacion']="Ok: No corresponde validar nota 'Afectación por Ensanche'"
            ...
            if "Linea de Edificacion Particularizada" in cur_afectaciones:
                if "Linea de Edificacion Particularizada" in particularidades:
                    if "Linea de Edificacion Particularizada".lower().replace(" ","") in (notas_0[0].lower()):
                        ...
                        validaciones2.loc[51,'Observacion']="Ok: Se indicó nota de 'Afectación por Linea de Edificacion Particularizada' en Notas del formulario de Mensura"
                    else:
                        validaciones2.loc[51,'Resultado']=-1
                        validaciones2.loc[51,'Observacion']="Error: No se indicó nota de 'Afectación por Linea de Edificacion Particularizada' en Notas del formulario de Mensura"
                else:
                    validaciones2.loc[51,'Resultado']=-1
                    validaciones2.loc[51,'Observacion']="Error: No se indicó nota de 'Afectación por Linea de Edificacion Particularizada' en Particularidades del formulario de Mensura"
            else:
                validaciones2.loc[51,'Resultado']=99
                validaciones2.loc[51,'Observacion']="Ok: No corresponde validar nota 'Afectación por Linea de Edificacion Particularizada'"       
...
def cur_parcela():
    ...
    try:
        resp_parc_datos_0 = requests.get(f"https://epok.buenosaires.gob.ar/cur3d/seccion_edificabilidad/?smp={smp[0]}")                            
        resp_parc_datos = json.loads(resp_parc_datos_0.text)
