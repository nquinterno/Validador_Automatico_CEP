...
def Procesar_Archivo():
    ...
    if doc != "":
        ...
        btn_resumen = Button(marco8,text="Resumen DXF", command=mostrar_resumen)
        btn_resumen.configure(font=("Gotham Rounded",8))
        btn_resumen.grid(padx=5, pady=2, row = 1, column=1)

        btn_descargar = Button(marco7,text="Descargar DXF", command=descargar_dxf)
        btn_descargar.configure(font=("Gotham Rounded",8))
        btn_descargar.grid(padx=5, pady=2, row = 0, column=1)

        btn_pdf = Button(marco8,text="Descargar PDF", command=salida_pdf)
        btn_pdf.configure(font=("Gotham Rounded",8))
        btn_pdf.grid(padx=5, pady=2, row = 1, column=0)
...
# abrir_enc = Button(marco3, text="Abrir Encomienda", command=Abrir_Archivo_ENCOMIENDA)
# abrir_enc.configure(font=("Gotham Rounded",8))
# abrir_enc.grid(padx=5, pady=2, row = 3, column=0)

abrir_IFTAM = Button(marco6, text="Abrir Form Mensura", command=Abrir_Archivo_IFTAM)
abrir_IFTAM.configure(font=("Gotham Rounded",8))
abrir_IFTAM.grid(padx=5, pady=2, row = 0, column=0)

abrir_IFFVN = Button(marco6, text="Abrir Form Resumen", command=Abrir_Archivo_IFFVN)
abrir_IFFVN.configure(font=("Gotham Rounded",8))
abrir_IFFVN.grid(padx=5, pady=2, row = 1, column=0)

abrir_IFDOM = Button(marco6, text="Abrir Form Dominio", command=Abrir_Archivo_IFDOM)
abrir_IFDOM.configure(font=("Gotham Rounded",8))
abrir_IFDOM.grid(padx=5, pady=2, row = 2, column=0)

abrir_FOMUBI = Button(marco6, text="Abrir Form. Ubicacíon", command=Abrir_Archivo_FOMUBI)
                ...
                if parcela.has_arc:
                    ...
                    for seg in segmentos:
                        ...
                        if seg.dxftype()== "ARC":
                            ...
                            angle = abs(end_angle - start_angle) #% (2 * math.pi)
...
##                lados_parcelas_l.append(parcela.dxf.count)
    ...
    patron_parc = re.compile('(^([0-9]{3})|([0-9]{3}[a-z]{1})|([0-9]{3}[ñ]{1})|([0-9]{2}[l]{2}))$') #patron para validar el formato de la parcela escrito en el bloque mensura
    nom_parc_list=list()
    ...
    if len(bloque_PARCELA_SURGENTE)==1:
            ...
            for bloque in (bloque_PARCELA_SURGENTE):
                    ...
                    if -1 in ban_bparc_comp:
                        ...
                        validaciones2.loc[15,'Resultado']=-1
                        validaciones2.loc[15,'Observacion']='ERROR: Se modificó el bloque PARCELA_SURGENTE, utilice el de la plantilla.'
                        validaciones2.loc[15,'Cetegoría']='Bloques' 
        ...
        if "-1" in band_poly_mej or "-2" in band_poly_mej: # si hay un -1 en la bandera arroja error 
            ...
            for mejora in mejoras_poly: #reccorre poligonos de mejoras
                ...
                if '0' in band_parc_dentro:
                    ...
                    if mejora.dxf.color ==4 or mejora.dxf.color ==5 or mejora.dxf.color ==216 or mejora.dxf.color == 155:
                        band_mej_dentro.append('0') #si la polilinea es cian es porque es balcon, entonces puede caer fuera de la parcela
                    else:
                        if mejora.dxf.layer != "M-M-PB-SUP": #si se equivoco y le puso otro color pero no esta en PB 
                            band_mej_dentro.append('0')
                        else:
            ...
            if ('-1' in band_mej_dentro) and not("1" in band_parc_arc_1):
...
def chequeo_layout():
    ...
    if len(smp)==1:
        ...
        if len(bloque_exp)==1:
            ...
            if -1 in ban_exp:
                validaciones2.loc[29,'Resultado']=50
                validaciones2.loc[29,'Observacion']='Verificar: El bloque "expediente" tiene campos vacíos'
                validaciones2.loc[29,'Cetegoría']='Bloques'
            
            elif -2 in ban_exp:
                validaciones2.loc[29,'Resultado']=50
                validaciones2.loc[29,'Observacion']='Verificar: Se ha modificado la configuración del bloque "expediente"'
                validaciones2.loc[29,'Cetegoría']='Bloques'

            else:
                validaciones2.loc[29,'Resultado']=0
                validaciones2.loc[29,'Observacion']='Ok: El bloque "expediente" tiene campos completos'
                validaciones2.loc[29,'Cetegoría']='Bloques' 
            ...
            validaciones2.loc[29,'Resultado']=50
            validaciones2.loc[29,'Observacion']=f'Verificar: Se ha insertado más de un bloque "expediente" en el layout {smp[0]} '
            validaciones2.loc[29,'Cetegoría']='Bloques'
            ...
            if len(text_exp)==1:
                ...
                band_tex_exp = "-1"
            validaciones2.loc[29,'Resultado']=50
            validaciones2.loc[29,'Observacion']=f'Verificar: No se ha insertado el bloque "expediente" en el layout {smp[0]}'
            validaciones2.loc[29,'Cetegoría']='Bloques'
        ...
        if band_limits == -1:
            ...
            validaciones2.loc[30,'Observacion']="ERROR: El layout con nombre smp debe medir 17 x 17cm"
            validaciones2.loc[30,'Cetegoría']='Layout'
        else:
            validaciones2.loc[30,'Resultado']=0 # si hay un -1 en la bandera arroja ERROR 
            validaciones2.loc[30,'Observacion']="OK: El layout con nombre smp mide 17 x 17cm"
            validaciones2.loc[30,'Cetegoría']='Layout'
    
    else:
        validaciones2.loc[27,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
        validaciones2.loc[27,'Observacion']= "Error: No se puden validar los layers frizados por existir mas de un layout con el formato sss-mmm-ppp"
        validaciones2.loc[27,'Cetegoría']='Layout'
        ...
        validaciones2.loc[29,'Observacion']="Error: No se pude validar el bloque 'expediente' por existir mas de un layput con el formato sss-mmm-ppp"
        validaciones2.loc[29,'Cetegoría']='Layout'

        validaciones2.loc[30,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
        validaciones2.loc[30,'Observacion']="Error: No se pude validar tamaño de layout por existir mas de un layout con el formato sss-mmm-ppp"
        validaciones2.loc[30,'Cetegoría']='Layout'
...
def georref():
    ...
    if len(smp)==1:
        ...
        validaciones2.loc[31,'Observacion']="Ok: Existe un unico Layout de Ficha catastral con nomenclatura en su nombre SSS-MMMM-PPPP"
        ...
        try:
            ...
            validaciones2.loc[33,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
            validaciones2.loc[33,'Observacion']="Error: No se puede validar la georref., verifique su conexión, que la parcela se encuentre en Ciudad3d, o la nomenclatura del layout"
            validaciones2.loc[33,'Cetegoría']='Tolerancias'
            ...
            if len(parcelas_poly):
                ...
                for vert in ver_cep_p2:
                    ...
                    if "-1" in band_tol_m_error:
                        ...
                        validaciones2.loc[32,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
                        validaciones2.loc[32,'Observacion']="Error: No se ha georreferenciado Correctamente"
                        validaciones2.loc[32,'Cetegoría']='Tolerancias'
                    else:
                        if "-1" in band_tol_m_verif:
                            validaciones2.loc[32,'Resultado']=50# si hay un -1 en la bandera arroja error 
                            validaciones2.loc[32,'Observacion']=f"Verificar: La parcela medida excede la tolerancia de {tol_verif_m}m respecto a la manzana antecedente de Ciudad 3d, verificar georreferenciación"
                            validaciones2.loc[32,'Cetegoría']='Tolerancias'
                        else:
                            validaciones2.loc[32,'Resultado']=0 # si hay un -1 en la bandera arroja error 
                            validaciones2.loc[32,'Observacion']="Ok: La parcela se encuentra dentro de la manzana a la que pertenece"
                            validaciones2.loc[32,'Cetegoría']='Tolerancias'
                    
                    if "-1" in  band_tol_p_error:
                            validaciones2.loc[33,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
                            validaciones2.loc[33,'Observacion']=f"Error: La parcela medida excede la tolerancia de {tol_error_p} respecto a la parcela antecedente, no se ha georreferenciado correctamente"
                            validaciones2.loc[33,'Cetegoría']='Tolerancias'
                    else:
                        if "-1" in band_tol_p_verif:
                            validaciones2.loc[33,'Resultado']=50 # si hay un -1 en la bandera arroja error 
                            validaciones2.loc[33,'Observacion']=f"Verificar: La parcela medida excede la tolerancia de {tol_verif_p}m respecto a la parcela antecedente de Ciudad 3d, cerificar medidas con ficha parcelaria"
                            validaciones2.loc[33,'Cetegoría']='Tolerancias'
                        else:
                            validaciones2.loc[33,'Resultado']=0 # si hay un -1 en la bandera arroja error 
                            validaciones2.loc[33,'Observacion']="Ok: La parcela se encuentra en tolerancia con respecto a la parcela antecedente de Ciudsd 3d"
                            validaciones2.loc[33,'Cetegoría']='Tolerancias'
                ...
                #                 validaciones2.loc[33,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
                #                 validaciones2.loc[33,'Observacion']="Error: No se ha georreferenciado Correctamente"
                #                 validaciones2.loc[33,'Cetegoría']='Tolerancias'
                            
                #             else:

                #                 validaciones2.loc[32,'Resultado']=50 # si hay un -1 en la bandera arroja error 
                #                 validaciones2.loc[32,'Observacion']="Verificar: La parcela medida difiere en mas de 50cm de la manzana catastral"
                #                 validaciones2.loc[32,'Cetegoría']='Tolerancias'
                ...
                #     else:
                #         validaciones2.loc[32,'Resultado']=0 # si hay un -1 en la bandera arroja error 
                #         validaciones2.loc[32,'Observacion']="Ok: La parcela medida se encuentradentro de la manzana a la cual pertenece"
                #         validaciones2.loc[32,'Cetegoría']='Tolerancias'
                ...
                #             if "-1" in band_tol_p6:
                #                 validaciones2.loc[33,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
                #                 validaciones2.loc[33,'Observacion']="Error: La parcela medida excede la tolerancia admitida respecto a la parcela antecedente o no se ha georreferenciado correctamente"
                #                 validaciones2.loc[33,'Cetegoría']='Tolerancias'
                #             else:
                #                 validaciones2.loc[33,'Resultado']=50 # si hay un -1 en la bandera arroja error 
                #                 validaciones2.loc[33,'Observacion']="Verificar: La parcela medida excede la tolerancia admitida respecto a la parcela antecedente de Ciudad 3d"
                #                 validaciones2.loc[33,'Cetegoría']='Tolerancias'
                #         else:
                #             validaciones2.loc[33,'Resultado']=50 # si hay un -1 en la bandera arroja error 
                #             validaciones2.loc[33,'Observacion']="Verificar: La parcela medida excede la tolerancia admitida respecto a la parcela antecedente de Ciudad 3d"
                ...
                validaciones2.loc[33,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
                validaciones2.loc[33,'Observacion']="Error: No se puede validar la georreferenciación porque no se encuentra dubujada la parcela como polilinea cerrada en el layer 09-M-PARCELA"
                validaciones2.loc[33,'Cetegoría']='Tolerancias'
        else:
                ...
                validaciones2.loc[33,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
                validaciones2.loc[33,'Observacion']="Error: No puede validarse la georref., no se encuentra en Ciudad3d la parcela indicada en el layout o no se puede conectarse con Ciudad3d"
                validaciones2.loc[33,'Cetegoría']='Tolerancias'
   
    
    elif len(smp)>1:
        validaciones2.loc[31,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
        validaciones2.loc[31,'Observacion']="Error: Existen mas de un Layout de Ficha catastral con nomenclatura en su nombre SSS-MMMM-PPPP"
        validaciones2.loc[31,'Cetegoría']='Layout'
        ...
        validaciones2.loc[33,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
        validaciones2.loc[33,'Observacion']="Error: Existen más de un Layout con nombre SSS-MMMM-PPPP, con lo que no es posible validar las diferencias entre parcela medida y antecedente"
        validaciones2.loc[33,'Cetegoría']='Layout'

    else:

        validaciones2.loc[31,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
        validaciones2.loc[31,'Observacion']="Error: No existe layout cuyo nombre tenga formato de nomenclatura catastral SSS-MMMM-PPPP"
        validaciones2.loc[31,'Cetegoría']='Layout'
        ...
        validaciones2.loc[33,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
        validaciones2.loc[33,'Observacion']="Error: No existe layout cuyo nombre tenga formato ‘SSS-MMMM-PPPP’, no es posible validar las diferencias entre parcela medida y antecedente"
        validaciones2.loc[33,'Cetegoría']='Layout'
...
def resumen():
    ...
    global pages_IFDOM_text
    global pages_FOMUBI_text
    # global pages_ENCOMIENDA_CPA_text
    # global pages_ENCOMIENDA_CPIC_text
    global piso_con_mejoras
    global info_form_emp
    global info_form_nuevo
    ...
    sup_arco = 0
    if len(parcelas_poly):
        area_parc_dxf_0 = round(ezdxf.math.area(Vec2.list(parcelas_poly[0].get_points('xy'))),2)
        if len(sup_arc_parc):
            for sup in sup_arc_parc:
                sup_arco = sup_arco + sup
            area_parc_dxf = area_parc_dxf_0 + sup_arco
        else:
           area_parc_dxf = area_parc_dxf_0
    else:
        area_parc_dxf = 0
    ...
    if len(pages_IFTAM_text)>0 and len(pages_IFFVN_text)>0 and len(pages_IFDOM_text)>0 and len(pages_FOMUBI_text)>0:
        ...
        if len(smp)>0:
            if smp_fomubi == smp[0]:
                if parc_fomubi == nom_parc_list[0]: # completar con parcela del atributo de bloque de la parc
                    ...
                    validaciones2.loc[34,'Observacion']="Ok: Coinciden las nomenclaturas de Caratula del Exp, Layout y bloque parc. surg"
                else:
                    validaciones2.loc[34,'Resultado']=-1
                    validaciones2.loc[34,'Observacion']="Error: No coincide la parcela del Layout y bloque parc. surg"
            else:
                validaciones2.loc[34,'Resultado']=-1
                validaciones2.loc[34,'Observacion']="Error: No coincide la parcela del Layout y La caratula del Exp"
        else:
            validaciones2.loc[34,'Resultado']=-1
            validaciones2.loc[34,'Observacion']="Error: No se indico SMP en el layout de ficha catastral o no tiene el formato correcto ej: '003-134A-001b'"
        print("exp_layout")
        print(exp_layout)
        if band_exp_fomubi =="0":
            if exp_fomubi[:16] == exp_layout[:16]: # completar con la variable que tenga el numero de exp tomado del layout
                ...
                validaciones2.loc[35,'Observacion']="Ok: Coinciden las nomenclaturas de Caratula del Exp, Layout y bloque parc. surg"
            else:
                validaciones2.loc[35,'Resultado']=-1
                validaciones2.loc[35,'Observacion']="Error: No Coincide el Expediente del tramite con el declarado en el layout"
        else:
            validaciones2.loc[35,'Resultado']=50
            validaciones2.loc[35,'Observacion']="Verificar: No se pudo Leer el número de Exp del Form. de ubic., verifique manualmente el exp del croquis de parcela"

        if abs(dif_mensura_dxf) > 0.01:
            if not("1" in band_parc_arc):
                validaciones2.loc[36,'Resultado']=-1
                validaciones2.loc[36,'Observacion']="Error: No coincide la Superficie de la Parcela en el dxf con la declarada en el formulario de Mensura"
            else:
                validaciones2.loc[36,'Resultado']=50
                validaciones2.loc[36,'Observacion']="Verificar: La parcela tiene lado curvo, verif. la sup. del form de mensura y la del dxf manualmente" 
        else:
            validaciones2.loc[36,'Resultado']=0
            validaciones2.loc[36,'Observacion']="Ok: Coinciden la Superficie de la Parcela en el dxf con la declarada en el formulario de Mensura"

        if dif_plantas_dxf != 0:
            validaciones2.loc[37,'Resultado']=-1
            validaciones2.loc[37,'Observacion']="Error: No coincide la N° de Plantas en las que se dibujó en el dxf con el N° de plantas declaradas en el form de mensura"
        else:
            validaciones2.loc[37,'Resultado']=0
            validaciones2.loc[37,'Observacion']="Ok: Coincide la N° de Plantas en las que se dibujó en el dxf con el N° de plantas declaradas en el form de mensura"

        if abs(dif_cubierta_dxf) > 0.01 and len(band_mejora_cub_arc_0)==0:
            validaciones2.loc[38,'Resultado']=-1
            validaciones2.loc[38,'Observacion']="Error: No coincide la Sup. Cubierta dibujada en el dxf con la superficie cubierta declarada en el form de mensura"
        
        elif abs(dif_cubierta_dxf) > 0.01 and len(band_mejora_cub_arc_0)!=0:
            validaciones2.loc[38,'Resultado']=50
            validaciones2.loc[38,'Observacion']="Verificar: Algun poligono cubierto o poligono interno posee un lado de arco, verificar sup. cubierta de formularios y dxf manualmente"
        else:
            validaciones2.loc[38,'Resultado']=0
            validaciones2.loc[38,'Observacion']="Ok:Coincide la Sup. Cubierta dibujada en el dxf con la superficie cubierta declarada en el form de mensura"

        if abs(dif_semi_dxf) > 0.01 and len(band_mejora_semi_arc_0)==0:
            validaciones2.loc[39,'Resultado']=-1
            validaciones2.loc[39,'Observacion']="Error: No coincide la Sup. semicubierta dibujada en el dxf con la superficie semicubierta declarada en el form de mensura"
        
        elif abs(dif_semi_dxf) > 0.01 and len(band_mejora_semi_arc_0)!=0:
            validaciones2.loc[39,'Resultado']=50
            validaciones2.loc[39,'Observacion']="Verificar: Algun poligono semicub o poligono interno posee un lado de arco, verificar sup. cubierta de formularios y dxf manualmente"
        
        else:
            validaciones2.loc[39,'Resultado']=0
            validaciones2.loc[39,'Observacion']="Ok: Coincide la Sup. Semicubierta dibujada en el dxf con la superficie semicubierta declarada en el form de mensura"

        if abs(dif_desc_dxf) > 0.01 and len(band_mejora_desc_arc_0)==0:
            validaciones2.loc[40,'Resultado']=-1
            validaciones2.loc[40,'Observacion']="Error: No coincide la Sup. descubierta dibujada en el dxf con la superficie descubierta declarada en el form de mensura"
        
        elif abs(dif_desc_dxf) > 0.01 and len(band_mejora_desc_arc_0)!=0:
            validaciones2.loc[40,'Resultado']=50
            validaciones2.loc[40,'Observacion']="Verificar: Algun poligono descub o poligono interno posee un lado de arco, verificar sup. cubierta de formularios y dxf manualmente"
        
        else:
            validaciones2.loc[40,'Resultado']=0
            validaciones2.loc[40,'Observacion']="Ok: Coincide la Sup. descubierta dibujada en el dxf con la superficie descubierta declarada en el form de mensura"
        ...
        if lados_parcelas_l[0] == len(medidas):
            ...
            validaciones2.loc[41,'Observacion']="Ok: Coincide la cantidad de lados de la parcela del dxf con la cantidad de rumbos declarados en el form. de mensura"
            ...
            if "-1" in band_medida:
                validaciones2.loc[42,'Resultado']=-1
                validaciones2.loc[42,'Observacion']="Error: No coinciden las medida de lados declaradas en el form de mensura con las medidas de los lados de la parcela del dxf"
            else:
                validaciones2.loc[42,'Resultado']=0
                validaciones2.loc[42,'Observacion']="Ok: Coinciden las medida de lados declaradas en el form de mensura con las medidas de los lados de la parcela del dxf"
            
        else:
            validaciones2.loc[41,'Resultado']=-1
            validaciones2.loc[41,'Observacion']="Error: No coincide la cantidad de lados de la parcela del dxf con la cantidad de rumbos declarados en el form. de mensura"

            validaciones2.loc[42,'Resultado']=-1
            validaciones2.loc[42,'Observacion']="Error: No coincide la cantidad de lados de la parcela del dxf con la cantidad de rumbos declarados en el form. de mensura"

        if cont_3 > 0:

            if ("contravencion" in band_notas):
                validaciones2.loc[43,'Resultado']=0
                validaciones2.loc[43,'Observacion']="Ok: Se detectó nota de Superficie en contravención de los RT en el Campo 'Notas' del Form. de Mensura"
            else:
                validaciones2.loc[43,'Resultado']=-1
                validaciones2.loc[43,'Observacion']="Error: No se detectó nota de Superficie en contravención de los RT en el Campo 'Notas' del Form. de Mensura"
        else:
            validaciones2.loc[43,'Resultado']=99
            validaciones2.loc[43,'Observacion']="Ok: No corresponde validar nota de Superficie en contravención por no haberse declarado la misma"
        ...
        if (mensura_3 - sup_tit) >0:
            if (mensura_3 - sup_tit) > (0.05 * mensura_3):
                ...
                if ("excedente_1" in band_notas) or ("excedente_2" in band_notas):
                    ...
                    validaciones2.loc[44,'Observacion']="Ok: Se ha detectado alguna de las notas de Excedente de los RT en el campo 'Notas' del formulario tecnico de Mensura"
                else:
                    validaciones2.loc[44,'Resultado']=-1
                    validaciones2.loc[44,'Observacion']="Error: No se ha detectado alguna de las notas de Excedente de los RT en el campo 'Notas' del formulario tecnico de Mensura"
            else:
                validaciones2.loc[44,'Resultado']=99
                validaciones2.loc[44,'Observacion']="Ok: No corresponde validar campo 'Notas' y 'Particularidades' de excedente"
            ...
            if (abs((mensura_3 - sup_tit)) > (0.05 * mensura_3)):
                validaciones2.loc[44,'Resultado']=50
                validaciones2.loc[44,'Observacion']="Verificar: Sup. s/título de form de dominio es mayor a sup en form de mensura (mayor al 5%) verifique que sup. s/titulo se declaró correctamente"
        ...
        if "excedente" in (particularidades[0].lower().replace(" ","")):
            ...
            if "excedente_2" in band_notas:
                ...
                validaciones2.loc[45,'Observacion']="Ok: En particularidades se declaró excedente y se indico nota del mismo en el campo 'Notas' del form. tecnico de Mensura"
            else:
                validaciones2.loc[45,'Resultado']=0
                validaciones2.loc[45,'Observacion']="Error: En particularidades se declaró excedente y no se indico nota del mismo en el campo 'Notas' del form. tecnico de Mensura"
        else:
            validaciones2.loc[45,'Resultado']=99
            validaciones2.loc[45,'Observacion']="Ok: En particularidades no se declaró excedente, no corresponde validar nota de excedente en form. técnico de Mensura"

        if dif_agip =="no":

            if "agip_nodif" in band_notas:
                validaciones2.loc[46,'Resultado']=0
                validaciones2.loc[46,'Observacion']="Ok: se declaró que no difiere con agip y se detectó la nota correspondiente segun los RT en el campo notas del form. tecnico de mensura"
                
                validaciones2.loc[47,'Resultado']=0
                validaciones2.loc[47,'Observacion']="Ok: se declaró que no difiere con agip y se detectó la nota correspondiente segun los RT en el campo notas del form. tecnico de mensura"
            
            else:
                if "cep_demolicion" in band_notas:
                    validaciones2.loc[46,'Resultado']=0
                    validaciones2.loc[46,'Observacion']="Ok: se declaró que no difiere con agip y se detectó la nota de Cep demolicion en el campo notas del form. tecnico de mensura"

                    validaciones2.loc[47,'Resultado']=0
                    validaciones2.loc[47,'Observacion']="Ok: se declaró que no difiere con agip y se detectó la nota de Cep demolicion en el campo notas del form. tecnico de mensura"
                else:
                    validaciones2.loc[46,'Resultado']=-1
                    validaciones2.loc[46,'Observacion']="Error: se declaró que no difiere con agip y no se detectó la nota correspondiente segun los RT en el campo notas del form. tecnico de mensura"

                    validaciones2.loc[47,'Resultado']=-1
                    validaciones2.loc[47,'Observacion']="Error: se declaró que no difiere con agip y no se detectó la nota correspondiente segun los RT en el campo notas del form. tecnico de mensura"
        elif dif_agip =="si":

            if supdemo_3>0:
                if "agip_dif_menos" in band_notas:
                    validaciones2.loc[46,'Resultado']=0
                    validaciones2.loc[46,'Observacion']="Ok: se declaró que difiere con agip, superficie demolida y se detectó la nota correspondiente en el campo notas del form. tecnico de mensura"
                else:
                    validaciones2.loc[46,'Resultado']=-1
                    validaciones2.loc[46,'Observacion']="Error: se declaró que difiere con agip, superficie demolida y no se detectó la nota correspondiente segun los RT en el campo notas del form. tecnico de mensura"
            else:
                validaciones2.loc[46,'Resultado']=99
                validaciones2.loc[46,'Observacion']="Ok: No corresponde validar nota de demolicion por no haberse declarado la misma en el form resumen"
            ...
            if len(agip_supnueva)>0:
                ...
                if sum(agip_supnueva)>0:
                    if "agip_dif_mas" in band_notas:
                        validaciones2.loc[47,'Resultado']=0
                        validaciones2.loc[47,'Observacion']="Ok: se declaró que difiere con agip, superficie nueva y se detectó la nota correspondiente segun los RT en el campo notas del form. tecnico de mensura"
                    else:
                        validaciones2.loc[47,'Resultado']=-1
                        validaciones2.loc[47,'Observacion']="Error: se declaró que difiere con agip, superficie nueva y no se detectó la nota correspondiente segun los RT en el campo notas del form. tecnico de mensura"
                else:
                    validaciones2.loc[47,'Resultado']=99
                    validaciones2.loc[47,'Observacion']="Ok: No corresponde validar nota de superficie a empadronar por no haberse declarado superficie nueva en el from resumen"

            else:
                validaciones2.loc[47,'Resultado']=99
                validaciones2.loc[47,'Observacion']="Ok: se declaró que difiere con agip, y no se indicó ningun valor en el campo sup. nueva en el form. resumen"  
      
        else:
            validaciones2.loc[46,'Resultado']=-1
            validaciones2.loc[46,'Observacion']="Error: no se puede validar nota de agip por no haberse completado si difere o no con agip las construcciones relevadas"
            
            validaciones2.loc[47,'Resultado']=-1
            validaciones2.loc[47,'Observacion']="Error: no se puede validar nota de agip por no haberse completado si difere o no con agip las construcciones relevadas"
        
        if "-1" in band_cur_parcela:
            validaciones2.loc[48,'Resultado']=-1
            validaciones2.loc[48,'Observacion']="Error: No se puede comprobar APH y afectaciones de la parcela por no poder conectarse con Ciudad 3d, verifique conexión de internet"
        else:
            if "cep_demolicion" in band_notas:
                if aph != "No":
                    validaciones2.loc[48,'Resultado']=-1
                    validaciones2.loc[48,'Observacion']="Error: Se ha declarado CEP Demolición y la parcela esta afectada a APH"

                else:
                    validaciones2.loc[48,'Resultado']=0
                    validaciones2.loc[48,'Observacion']="Ok: Se ha declarado CEP Demolición y la parcela no esta afectada a APH"

            else:
                validaciones2.loc[48,'Resultado']=0
                validaciones2.loc[48,'Observacion']="Ok: No se ha declarado CEP Demolición"

            if aph != "No":
                if "aph" in band_notas:
                    validaciones2.loc[49,'Resultado']=0
                    validaciones2.loc[49,'Observacion']="Ok: Se ha indicado la Nota de Aph en formulario de Mensura"
                else:
                    validaciones2.loc[49,'Resultado']=-1
                    validaciones2.loc[49,'Observacion']="Error: No se ha indicado la Nota de Aph en formulario de Mensura y la parcela se encuentra catalogada"
            else:
                validaciones2.loc[49,'Resultado']=99
                validaciones2.loc[49,'Observacion']="Ok: No corresponde validar nota de Aph"
            ...
            if "Cinturon Digital" in cur_afectaciones:
                if "Cinturon Digital" in particularidades:
                    if "Cinturon Digital".lower().replace(" ","") in (notas_0[0].lower()):
                        ...
                        validaciones2.loc[50,'Observacion']="Ok: Se indicó nota de 'Afectación a Cinturon Digital' en Notas del formulario de Mensura"
                    else:
                        validaciones2.loc[50,'Resultado']=-1
                        validaciones2.loc[50,'Observacion']="Error: No se indicó nota de 'Afectación a Cinturon Digital' en Notas del formulario de Mensura"
                else:
                    validaciones2.loc[50,'Resultado']=-1
                    validaciones2.loc[50,'Observacion']="Error: No se indicó nota de 'Afectación a Cinturon Digital' en Particularidades del formulario de Mensura"
            else:
                validaciones2.loc[50,'Resultado']=99
                validaciones2.loc[50,'Observacion']="Ok: No corresponde validar nota de 'Afectación Cinturon Digital'"
            ...
            if "Afectación por Apertura" in cur_afectaciones:
                if "Afectación por Apertura" in particularidades:
                    if "Afectación por Apertura".lower().replace(" ","") in (notas_0[0].lower()):
                        validaciones2.loc[51,'Resultado']=0
                        validaciones2.loc[51,'Observacion']="Ok: Se indicó nota de 'Afectación por Apertura' en Notas del formulario de Mensura"
                    else:
                        validaciones2.loc[51,'Resultado']=-1
                        validaciones2.loc[51,'Observacion']="Error: No se indicó nota de 'Afectación por Apertura' en Notas del formulario de Mensura"
                else:
                    validaciones2.loc[51,'Resultado']=-1
                    validaciones2.loc[51,'Observacion']="Error: No se indicó nota de 'Afectación por Apertura' en Particularidades del formulario de Mensura"
            else:
                validaciones2.loc[51,'Resultado']=99
                validaciones2.loc[51,'Observacion']="Ok: No corresponde validar nota 'Afectación por Apertura'"
            ...
            if "Afectación por Ensanche" in cur_afectaciones:
                if "Afectación por Ensanche" in particularidades:
                    if "Afectación por Ensanche".lower().replace(" ","") in (notas_0[0].lower()):
                        validaciones2.loc[52,'Resultado']=0
                        validaciones2.loc[52,'Observacion']="Ok: Se indicó nota de 'Afectación por Ensanche' en Notas del formulario de Mensura"
                    else:
                        validaciones2.loc[52,'Resultado']=-1
                        validaciones2.loc[52,'Observacion']="Error: No se indicó nota de 'Afectación por Ensanche' en Notas del formulario de Mensura"
                else:
                    validaciones2.loc[52,'Resultado']=-1
                    validaciones2.loc[52,'Observacion']="Error: No se indicó nota de 'Afectación por Ensanche' en Particularidades del formulario de Mensura"
            else:
                validaciones2.loc[52,'Resultado']=99
                validaciones2.loc[52,'Observacion']="Ok: No corresponde validar nota 'Afectación por Ensanche'"
            ...
            if "Linea de Edificacion Particularizada" in cur_afectaciones:
                if "Linea de Edificacion Particularizada" in particularidades:
                    if "Linea de Edificacion Particularizada".lower().replace(" ","") in (notas_0[0].lower()):
                        validaciones2.loc[53,'Resultado']=0
                        validaciones2.loc[53,'Observacion']="Ok: Se indicó nota de 'Afectación por Linea de Edificacion Particularizada' en Notas del formulario de Mensura"
                    else:
                        validaciones2.loc[53,'Resultado']=-1
                        validaciones2.loc[53,'Observacion']="Error: No se indicó nota de 'Afectación por Linea de Edificacion Particularizada' en Notas del formulario de Mensura"
                else:
                    validaciones2.loc[53,'Resultado']=-1
                    validaciones2.loc[53,'Observacion']="Error: No se indicó nota de 'Afectación por Linea de Edificacion Particularizada' en Particularidades del formulario de Mensura"
            else:
                validaciones2.loc[53,'Resultado']=99
                validaciones2.loc[53,'Observacion']="Ok: No corresponde validar nota 'Afectación por Linea de Edificacion Particularizada'"
        if "cep_demolicion" in band_notas:
            if  cubierta_3 != 0:
                validaciones2.loc[54,'Resultado']=-1
                validaciones2.loc[54,'Observacion']="Error: Se declaró CEP Demolición y se declaró Superficie cubierta distinta de 0 en el form de Mensura'"
            else:
                validaciones2.loc[54,'Resultado']=0
                validaciones2.loc[54,'Observacion']="Ok: Se declaró CEP Demolición y se declaró Superficie cubierta igual a 0 en el form de Mensura'"

            if semi_3 !=0:
                validaciones2.loc[55,'Resultado']=-1
                validaciones2.loc[55,'Observacion']="Error: Se declaró CEP Demolición y se declaró Superficie semicubierta distinta de 0 en el form de Mensura'"
            else:
                validaciones2.loc[55,'Resultado']=0
                validaciones2.loc[55,'Observacion']="Ok: Se declaró CEP Demolición y se declaró Superficie semicubierta igual a 0 en el form de Mensura'"

            if des_3 !=0:
                validaciones2.loc[56,'Resultado']=-1
                validaciones2.loc[56,'Observacion']="Error: Se declaró CEP Demolición y se declaró Superficie descubierta distinta de 0 en el form de Mensura'"
            else:
                validaciones2.loc[56,'Resultado']=0
                validaciones2.loc[56,'Observacion']="Ok: Se declaró CEP Demolición y se declaró Superficie descubierta igual a 0 en el form de Mensura'"

            if plantas_3 !=0:
                validaciones2.loc[57,'Resultado']=-1
                validaciones2.loc[57,'Observacion']="Error: Se declaró CEP Demolición y se declaró cantidad de plantas edificadas distinta de 0 en el form de Mensura'"
            else:
                validaciones2.loc[57,'Resultado']=0
                validaciones2.loc[57,'Observacion']="Ok: Se declaró CEP Demolición y se declaró cantidad de plantas edificadas igual a 0 en el form de Mensura'"
        else:
            pass
            
    else:
        validaciones2.loc[34,'Resultado']=-1
        validaciones2.loc[34,'Observacion']="Error: No es posible validar los formularios por no haberse cargado los mismos"
...
def analisis_FOMUBI():
    ...
    sec_fomubi =  re.findall(r'Sección: (.*?)Manzana:', pages_FOMUBI_text)
    manz_fomubi =  re.findall(r'Manzana: (.*?)Parcela:', pages_FOMUBI_text)
    parc_fomubi_0 =  re.findall(r'Parcela: (.*?)Unidad Funcional:', pages_FOMUBI_text)
    ...
    if len(exp_fomubi_0):
        ...
        band_exp_fomubi = "-1"
    smp_fomubi = sec_fomubi[0] + "-" + manz_fomubi[0] + "-" + parc_fomubi_0[0]
    parc_fomubi = parc_fomubi_0[0]
...
def salida_pdf():
    ...
    if len(validaciones3)>35:
        ...
        c.drawString(b1+10, 580, f'{plantas_dxf}')
        ...
        c.drawString(b1+10, 560, f'{cubierta_dxf}')
        ...
        c.drawString(b1+10, 540, f'{semicubierta_dxf}')
        ...
        c.drawString(b1+10, 520, f'{descubierta_dxf}')
...
Boton_Chequear_layers= Button(marco7, text="Procesar DXF CEP", command=Procesar_Archivo)
