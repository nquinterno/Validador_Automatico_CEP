    ...
    if len(parcelas): #si hay entindades en el layer patrcela comienza la validacion de las mismas sino arroja error que no hay nada en ese layer
        ...
        if "-1" in band_poly_parc: # si hay un -1 en la bandera arroja error 
            ...
            if len (parcelas_poly)>0:
                medidas_dxf = lados_polilinea(parcelas_poly[0])
            else:
                medidas_dxf = 0
    ...
    if parcela.has_arc:
        ...
        for seg in segmentos:
            if seg.dxftype()== "ARC":
                ...
                #flecha = radio * (1-math.cos(angle/2))
                dx = start_point[0] - centro[0]
                dy = start_point[1] - centro[1]
                rotated_dx = dx * math.cos(angle/2) - dy * math.sin(angle/2)
                rotated_dy = dx * math.sin(angle/2) + dy * math.cos(angle/2)

                dx_1 = end_point[0] - centro[0]
                dy_1 = end_point[1] - centro[1]
                rotated_dx_1 = dx_1 * math.cos(angle/2) - dy_1 * math.sin(angle/2)
                rotated_dy_1 = dx_1 * math.sin(angle/2) + dy_1 * math.cos(angle/2)

                midpoint_arc_1 = (start_point[0]+rotated_dx, start_point[1]+rotated_dy)
                midpoint_arc_2 = (end_point[0]+rotated_dx_1, end_point[1]+rotated_dy_1)
                ...
                if abs(dif_angle_1) < abs(dif_angle_2):
    ...
    if parcela.has_arc:
        ...
        for seg in segmentos:
            if seg.dxftype()== "ARC":
                ...
                radio = arc.dxf.radius
                angle = abs(end_angle - start_angle)
                medidas_dxf.append(round(radio * angle,2))
            else:
                start = seg.dxf.start
                end = seg.dxf.end
                medidas_dxf.append(round(math.sqrt(((start[0]-end[0])**2)+((start[1]-end[1])**2)),2))
    else:
        vertices = parcela.get_points('xy')
        for i in range(len(vertices)):
            medidas_dxf.append(round(math.sqrt(((vertices[i][0]-vertices[i-1][0])**2)+((vertices[i][1]-vertices[i-1][1])**2)),2))
    return medidas_dxf
