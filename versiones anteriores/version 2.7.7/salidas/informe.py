...
def salida_pdf(opcion,validaciones2,parcelas_poly,parc_ant_posgba_2,manz_ant_posgba_2,smp,smp_fomubi,exp_layout,exp_fomubi, medidas_dxf, lados_iftam, cubierta_3, mensura_3, plantas_3, semi_3, des_3, cont_3,     area_parc_dxf, area_excedente_dxf, plantas_dxf, cubierta_dxf, semicubierta_dxf, descubierta_dxf, sup_emp_dxf, sup_nueva_dxf,version,resultado_final,manzana_poly):
    ...
    if len(manzana_poly)>0:
        for manzana in manzana_poly:
            if manzana.closed:
                manzana_poly_close = manzana
            else:
                pass
    else:
        pass
    ...
    if len(validaciones3)>41:
        ...
        if manz_ant_posgba_2 and parc_ant_posgba_2 and parcelas_poly:
            ...
            try:
                ...
                if len(manzana_poly_close)>0:
                    manz_ant_posgba_5 = manzana_poly_close.get_points('xy')
                    manz_ant_posgba_6 = manz_ant_posgba_5

                else:
                    manz_ant_posgba_5 =[[[0],[0]],[[100],[0]],[[100],[100]],[[0],[100]]]
            try:
                parc_ant_posgba_5 = eval(parc_ant_posgba_2)
                for x in parc_ant_posgba_5:
                    parc_ant_posgba_6.append((x[0][0],x[1][0]))
            except:
                parc_ant_posgba_5 =[[[0],[0]],[[100],[0]],[[100],[100]],[[0],[100]]]
