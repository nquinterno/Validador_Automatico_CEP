...
abrir_FOMUBI = Button(marco6, text="Abrir Form Ubicación", command=Abrir_Archivo_FOMUBI)
        ...
        if len(bloque_exp)==1:
            for bloque in (bloque_exp):
                ...
                if -1 in ban_exp:
                    ...
                    validaciones2.loc[29,'Observacion']='Error: El bloque "expediente" tiene campos vacíos'
                    validaciones2.loc[29,'Cetegoría']='Bloques'
                
                elif -2 in ban_exp:
                    validaciones2.loc[29,'Resultado']=-1
                    validaciones2.loc[29,'Observacion']='Error: Se ha modificado la configuración del bloque "expediente"'
                    validaciones2.loc[29,'Cetegoría']='Bloques'
            ...
            if len(text_exp)==1:
                for entitie in text_exp:
                    texto_expediente = entitie.dxf.text
                exp_layout = texto_expediente
            elif len(text_exp)>1:
            ...
            validaciones2.loc[29,'Resultado']=-1
            validaciones2.loc[29,'Observacion']=f'Error: Se ha insertado más de un bloque "expediente" en el layout {smp[0]} '
            validaciones2.loc[29,'Cetegoría']='Bloques'
            ...
            validaciones2.loc[29,'Resultado']=-1
            validaciones2.loc[29,'Observacion']=f'Error: No se ha insertado el bloque "expediente" en el layout {smp[0]}'
            validaciones2.loc[29,'Cetegoría']='Bloques' 
    ...
    if len(smp)==1:
        ...
        if len(respuesta_parc)>0 and len(respuesta_manz)>0: #si ciudad 3d responde con coordenadas se hace la validacion
            ...
            if len(parcelas_poly):
                ...
                ver_ant_p5 = list(ezdxf.math.offset_vertices_2d(ver_ant_p2,offset=2.00, closed=True))
                ver_ant_p6 = list(ezdxf.math.offset_vertices_2d(ver_ant_p2,offset=-2.00, closed=True))
                ...
                ver_ant_m4 = list(ezdxf.math.offset_vertices_2d(ver_ant_m2,offset=-0.51, closed=True))

                ver_ant_m5 = list(ezdxf.math.offset_vertices_2d(ver_ant_m2,offset=2.00, closed=True)) 
                ver_ant_m6 = list(ezdxf.math.offset_vertices_2d(ver_ant_m2,offset=-2.00, closed=True)) 
                ...
                for vert in ver_cep_p2:
                    ...
                    if ezdxf.math.is_point_in_polygon_2d(vert,ver_ant_p4,abs_tol=1e-4)==-1:
                        band_tol_p4.append("-1") #valida que la parcela caiga fuera del ofset menor de 0,5m de la parcela antecedente

                    else:
                        band_tol_p4.append("0")

                    if ezdxf.math.is_point_in_polygon_2d(vert,ver_ant_p5,abs_tol=1e-4)==-1:
                        band_tol_p5.append("-1") #valida que la parcela caiga fuera del ofset menor de 0,5m de la parcela antecedente

                    else:
                        band_tol_p5.append("0")

                    if ezdxf.math.is_point_in_polygon_2d(vert,ver_ant_p6,abs_tol=1e-4)==-1:
                        band_tol_p6.append("-1") #valida que la parcela caiga fuera del ofset menor de 0,5m de la parcela antecedente

                    else:
                        band_tol_p6.append("0")
                    ...
                    if ezdxf.math.is_point_in_polygon_2d(vert,ver_ant_m4,abs_tol=1e-4)==-1: #si esta validacion da -1 quiere decir que la parcela del cep es mas grande que la antecedente con una tolerancia de 51cm
                        band_tol_m4.append("-1")   #valida que la parcela caiga dentro del ofset mayor de 0,5m de la manzana antecedente

                    else:
                        band_tol_m4.append("0")

                    if ezdxf.math.is_point_in_polygon_2d(vert,ver_ant_m5,abs_tol=1e-4)==-1: #si esta validacion da -1 quiere decir que la parcela del cep es mas grande que la antecedente con una tolerancia de 51cm
                        band_tol_m5.append("-1")   #valida que la parcela caiga dentro del ofset mayor de 0,5m de la manzana antecedente

                    else:
                        band_tol_m5.append("0")

                    if ezdxf.math.is_point_in_polygon_2d(vert,ver_ant_m6,abs_tol=1e-4)==-1: #si esta validacion da -1 quiere decir que la parcela del cep es mas grande que la antecedente con una tolerancia de 51cm
                        band_tol_m6.append("-1")   #valida que la parcela caiga dentro del ofset mayor de 0,5m de la manzana antecedente

                    else:
                        band_tol_m6.append("0")                        
...
def resumen():
    ...
    if len(pages_IFTAM_text)>0 and len(pages_IFFVN_text)>0 and len(pages_IFDOM_text)>0 and len(pages_FOMUBI_text)>0:
        ...
        if exp_fomubi == exp_layout: # completar con la variable que tenga el numero de exp tomado del layout
            ...
            #else de no se completaron correctamente en el layput el año y el numero de exp
        #else de no se inserto el bloque de exp en el layout
        ...
        if dif_agip =="no":
            ...
            if len(agip_supnueva)>0:
                ...
                validaciones2.loc[47,'Resultado']=-1
                validaciones2.loc[47,'Observacion']="Error: se declaró que difiere con agip, y no se indicó ningun valor en el campo sup. nueva en el form. resumen"  
...
def analisis_IFFVN():
    ...
    if '¿Difiere con AGIP?: No' in pages_IFFVN_text:
        ...
        supdemo_2 = re.findall(r"Superficie Demolida: (\d+,\d+)", pages_IFFVN_text)[0].replace(",", ".")
        if len(supdemo_2)>0: 
            supdemo_3 = float(supdemo_2)
        else:
            supdemo_3
...
def salida_pdf():
    ...
    if len(validaciones3)>0:
        ...
        c.drawString(b1+10, 660, f'{smp[0]}')
