...
def georref(smp,parcelas_poly_close):
            ...
            if len(parcelas_poly_close):
                ...
                if "-1" in  band_tol_p_error:
                    ...
                    if "-1" in band_tol_p_verif:
                        ...
                        validaciones_georref.loc[2,'Observacion']=f"Verificar: La parcela medida excede la tolerancia de {tol_verif_p}m respecto a la parcela antecedente de Ciudad 3d, cerificar medidas con ficha parcelaria"
                        validaciones_georref.loc[2,'Cetegoría']='Tolerancias'
                    else:
                        validaciones_georref.loc[2,'Resultado']=0 # si hay un -1 en la bandera arroja error 
                        validaciones_georref.loc[2,'Observacion']="Ok: La parcela se encuentra en tolerancia con respecto a la parcela antecedente de Ciudsd 3d"
                        validaciones_georref.loc[2,'Cetegoría']='Tolerancias'
