...
##    Calcular la cantidad de lados que tiene la parcela
 ...
 #valida que haya la misma cantidad de bloques de parcela insertos en el dxf que la cantidad de parcelas del layer parcelas
        ...
        for i in range (len(mejora_piso)): #recorre cada poligono dentro del piso
            ...
            if len(mejora_adentro_2)>0:
                ...
                for lado in mejora_piso[i].virtual_entities():
                    vert_pol_madre_arc = ""
                    if lado.dxftype() == "ARC":
                        vert_pol_madre_arc = discretizar_curva(lado,0.01)
                    else:
                        vert_pol_madre.append([round(lado.dxf.start[0],3),round(lado.dxf.start[1],3)])
                        vert_pol_madre.append([round(lado.dxf.end[0],3),round(lado.dxf.end[1],3)])
                    if vert_pol_madre_arc != "":
                        vert_pol_madre = vert_pol_madre + vert_pol_madre_arc
                    else:
                        pass

                
                for lado in mejora_adentro_2[0].virtual_entities():
                    vert_pol_agujero_arc = ""
                    if lado.dxftype() == "ARC":
                        vert_pol_agujero_arc = discretizar_curva(lado,0.01)
                    else:
                        vert_pol_agujero.append([round(lado.dxf.start[0],3),round(lado.dxf.start[1],3)])
                        vert_pol_agujero.append([round(lado.dxf.end[0],3),round(lado.dxf.end[1],3)])
                    
                    if vert_pol_agujero_arc != "":
                        vert_pol_agujero = vert_pol_agujero + vert_pol_agujero_arc
                    else:
                        pass
    ...
    if len(mejoras): #si hay entindades en el layer mejoras comienza la validacion de las mismas sino arroja error que no hay nada en ese layer
        ...
        if "-1" in band_poly_mej or "-2" in band_poly_mej: # si hay un -1 en la bandera arroja error 
            ...
            if ('-1' in band_mej_dentro) and not("1" in band_parc_arc_1):
                validaciones_model.loc[17,'Resultado']=-1 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
                validaciones_model.loc[17,'Observacion']="Error: Existe Polígono de superficie fuera de los limites de Parcela"
                validaciones_model.loc[17,'Cetegoría']='Mejoras'
            else:
                validaciones_model.loc[17,'Resultado']=0 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
                validaciones_model.loc[17,'Observacion']="OK: El ol los polígonos de superficie estan completamente contenidos dentro de la Parcela"
                validaciones_model.loc[17,'Cetegoría']='Mejoras'
    ...
    return validaciones_model, medidas_dxf, parcelas_poly_close, parcelas_poly, excedentes_poly,mejoras_dict, band_dict, nom_parc_list
