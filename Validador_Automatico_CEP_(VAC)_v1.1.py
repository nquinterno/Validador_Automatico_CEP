...
marco1 = Frame(raiz)
marco1.config(width="1014",height="250")
marco1.grid(padx=5, pady=5, row = 0, column=0)

marco2 =LabelFrame(raiz,text='Validacion')
marco2.grid(padx=5, pady=1, row = 2, column=0)
marco2.config(bg="white",width="1014",height="400")

##marco3 = Frame(marco1)
##marco3.config(width="340",height="20")
##marco3.grid(padx=25, pady=10, row = 3, column=0)

marco4 = Frame(raiz)
marco4.config(width="1014",height="20")
marco4.grid(padx=25, pady=3, row = 1, column=0)

##marco5 = Frame(marco1)
##marco5.config(width="340",height="20")
##marco5.grid(padx=25, pady=10, row = 3, column=1)

label = Label(marco1,text="VAC - (Validador Automatico CEP)")
label.configure(font=("Gotham Rounded",12))
label.grid(padx=25, pady=3, row = 0, column=1)
...
label1.configure(font=("Gotham Rounded",10))
label1.grid(padx=25, pady=3, row = 1, column=1)



##marco6 = Frame(marco1)
##marco6.configure()
##marco6.grid(padx=25, pady=10, row = 2, column=0)
##
##marco7 = Frame(marco1)
##marco7.configure()
##marco7.grid(padx=25, pady=10, row = 2, column=3)

#Boton_atras= Button(marco6, text="<", command="pagina_atras").pack()

#Boton_adelante= Button(marco7, text=">", command="pagina_adelante").pack()

##label2 = Label(marco1,text="Gerencia Operativa de Catastro Físico)")
##label2.configure(font=("Gotham Rounded",10))
##label2.grid(padx=25, pady=10, row = 3, column=1)
##
##label3 = Label(marco1,text="Dirección General de Registro de Obras y Catastro")
##label3.configure(font=("Gotham Rounded",10))
##label3.grid(padx=25, pady=10, row = 4, column=1)

tv = ttk.Treeview(marco2, height=27)
tv['columns']=("Nº","Validacion")
tv.column("#0", width="0", stretch="NO")
tv.column("Validacion",anchor="w", width="970")
...
def Procesar_Archivo():
    ...
    btn_resumen.configure(font=("Gotham Rounded",10))
    btn_resumen.grid(padx=25, pady=5, row = 1, column=1)

    btn_descargar = Button(marco4,text="Descargar vectores DXF", command=descargar_dxf)
    btn_descargar.configure(font=("Gotham Rounded",10))
    btn_descargar.grid(padx=25, pady=5, row = 1, column=2)


Boton_abrir= Button(marco4, text="Abrir archivo", command=Abrir_Archivo)
Boton_abrir.configure(font=("Gotham Rounded",10))
Boton_abrir.grid(padx=25, pady=5, row = 1, column=0)
...
##                lados_parcelas_l.append(parcela.dxf.count)
        ...
        if '0' in band_poly_color_parc and '0' in band_poly_gro_parc:
            ...
            validaciones2.loc[8,'Observacion']="ERROR: Grosor de polylineas erroneo en layer 09-M-PARCELA"
            ...
            validaciones2.loc[8,'Observacion']="ERROR: Color de polylineas erroneo en layer 09-M-PARCELA"
            validaciones2.loc[8,'Cetegoría']='Parcela/s'  
    ...
    if len(m_manzana)>0:
        for element in m_manzana:
            ...
            validaciones2.loc[9,'Observacion']="ERROR: El layer 07-M-MANZANA se encuentra vacío, debe copiarse con coordenadas originales los elmentos del DXF  de la manzana correspondiente"
    ...
    if "0" in band_ant_manzana:
        ...
        validaciones2.loc[9,'Resultado']=-1
        validaciones2.loc[9,'Observacion']="ERROR: En el layer 07-M-MANZANA no se ha insertado la manzana, verifique que haya pegado con coordenadas originales los elementos del dxf de la manzana correspondiente"
        validaciones2.loc[9,'Cetegoría']='Antecedente'
    ...
    if len(m_calles)>0:
        ...
        if "-1" in band_calles_text:
            ...
            validaciones2.loc[10,'Observacion']="ERROR: En el layer 05-M-NOMBRES-DE-CALLE deben indicarse solo los nombres de calle del DXF de la manzana correspondiente con entidad 'TEXT'"
        ...
        validaciones2.loc[10,'Resultado']=-1
        validaciones2.loc[10,'Observacion']="ERROR: El layer 05-M-NOMBRES-DE-CALLE se encuentra vacío, debe copiarse con coordenadas originales los elmentos del DXF  de la manzana correspondiente"
        validaciones2.loc[10,'Cetegoría']='Antecedente'
    ...
    if len(excedentes): #si hay entindades en el layer patrcela comienza la validacion de las mismas sino arroja error que no hay nada en ese layer
        ...
        validaciones2.loc[11,'Observacion']="OK: Validacion de excedente no corresponde por encontrase el layer vacío, verifique que no se constituya excedente o ya exista uno para la/s parcela mensuradas"
        validaciones2.loc[11,'Cetegoría']='Parcela/s'

        validaciones2.loc[12,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones2.loc[12,'Observacion']="OK: Validacion de excedente no corresponde por encontrase el layer vacío, verifique que no se constituya excedente o ya exista uno para la/s parcela mensuradas"
        validaciones2.loc[12,'Cetegoría']='Parcela/s'

        validaciones2.loc[13,'Resultado']=99 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
        validaciones2.loc[13,'Observacion']="OK: Validacion de excedente no corresponde por encontrase el layer vacío, verifique que no se constituya excedente o ya exista uno para la/s parcela mensuradas"
        validaciones2.loc[13,'Cetegoría']='Parcela/s'
...
##    for layer in nom_layers_dxf:
##        if "-SUP" in layer:
##            layers_mejoras.append(layer)
##        else:
##            pass
##    for layer in layers_mejoras:
##        
##        mejora_piso = doc.modelspace().query('LWPOLYLINE[layer ? f".*{layer}"]')
##        form_piso = doc.modelspace().query('INSERT[layer ? f".*{layer}"]')
##
##        if len(mejora_piso)==len(form_piso):
##            band_piso.append("0")
##        elif len(mejora_piso)>len(form_piso):
##            band_piso.append("-1")
##        elif len(mejora_piso)<len(form_piso):
##            band_piso.append("-2")

##    if "-1" in band_piso or "-2" in band_piso:
##        validaciones2.loc[26,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
##        validaciones2.loc[26,'Observacion']="Error: En alguno de los pisos hay mas polilineas cerradas de mejoras que bloques de formulario insertos"
##        validaciones2.loc[26,'Cetegoría']='Layout'
##    else:
##        validaciones2.loc[26,'Resultado']=0 # si hay un -1 en la bandera arroja error 
##        validaciones2.loc[26,'Observacion']="Ok: En todos los pisos existe la misma cantidad de polilineas cerradas de mejoras que bloques de formulario insertos"
##        validaciones2.loc[26,'Cetegoría']='Layout'
        
    for mejora in mejoras:
        layers_mejoras_0.append(mejora.dxf.layer)

    layers_mejoras = list(set(layers_mejoras_0))
    ...
    if len(mejoras): #si hay entindades en el layer mejoras comienza la validacion de las mismas sino arroja error que no hay nada en ese layer
        ...
        if "-1" in band_poly_mej or "-2" in band_poly_mej: # si hay un -1 en la bandera arroja error 
            ...
            for mejora in mejoras_poly:
                if mejora.dxf.color == 1 or mejora.dxf.color == 2 or mejora.dxf.color == 3:   
            ...
            if '0' in band_poly_color_mej:
                ...
                validaciones2.loc[17,'Observacion']="Error: Color de polylineas erroneo en los layers de '-SUP'"
                validaciones2.loc[17,'Cetegoría']='Mejoras'
            ...
            if '-1' in band_mej_dentro:
                ...
                validaciones2.loc[19,'Observacion']="Error: Existe Mejora fuera de los limites de Parcela"
                validaciones2.loc[19,'Cetegoría']='Mejoras'
            else:
                validaciones2.loc[19,'Resultado']=0 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
                validaciones2.loc[19,'Observacion']="OK: La o las Mejoras estan completamente contenidas dentro de la Parcela"
                validaciones2.loc[19,'Cetegoría']='Mejoras'
        ...
        validaciones2.loc[16,'Observacion']="OK: Validacion de Mejoras no corresponde por encontrase los layers de '-SUP' vacío, verifique que se trate de un inmueble baldío"
        validaciones2.loc[16,'Cetegoría']='Mejoras'

        validaciones2.loc[17,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones2.loc[17,'Observacion']="OK: Validacion de Mejoras no corresponde por encontrase los layers de '-SUP' vacío, verifique que se trate de un inmueble baldío"
        validaciones2.loc[17,'Cetegoría']='Mejoras'

        validaciones2.loc[18,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones2.loc[18,'Observacion']="OK: Validacion de Mejoras no corresponde por encontrase los layers de '-SUP' vacío, verifique que se trate de un inmueble baldío"
        validaciones2.loc[18,'Cetegoría']='Mejoras'

        validaciones2.loc[19,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones2.loc[19,'Observacion']="OK: Validacion de Mejoras no corresponde por encontrase los layers de '-SUP' vacío, verifique que se trate de un inmueble baldío"
        validaciones2.loc[19,'Cetegoría']='Mejoras'
    ...
    if (len(forms)>0 or len(forms_empadronados)>0):
        ...
        if len(forms_polig)>0:
            ...
            validaciones2.loc[20,'Observacion']="OK: No corresponde validar numeración de poligonos por no haberse insertado bloques 'forms' o no haberse completado el campo número de poligono del mismo"
            validaciones2.loc[20,'Cetegoría']='Formularios'

            validaciones2.loc[21,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
            validaciones2.loc[21,'Observacion']="OK: No corresponde validar numeración de poligonos por no haberse insertado bloques 'forms' o no haberse completado el campo número de poligono del mismo"
            validaciones2.loc[21,'Cetegoría']='Formularios'
...
def georref():
    ...
    if len(smp)==1:
        ...
        try:
            ...
            validaciones2.loc[23,'Observacion']="Error: No se puede validar la georref. porque no puede establecerse conexión con Ciudad3d, verifique su conexión a internet, la nomenclatura del layout o intente mas tarde"
            validaciones2.loc[23,'Cetegoría']='Tolerancias'

            validaciones2.loc[24,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
            validaciones2.loc[24,'Observacion']="Error: No se puede validar la georref. porque no puede establecerse conexión con Ciudad3d, verifique su conexión a internet, la nomenclatura del layout o intente mas tarde"
...
##            for i in range (len(parc_ant_wgs84)):  ##transforma las coordenadas de la parcela antecedente de wgs84 a posgar caba para poder comparar con la parcela del profesional
##                x,y = (parc_ant_wgs84[i])
##                print(x,y)
##                #parc_ant_posgba.append(transform(wgs84, posgba, x, y))
##                b,a = transformer.transform(y,x)
##                c = list()
##                c.append(a)
##                c.append(b)
##                parc_ant_posgba.append(c)
                
##            print("parc_ant_posgba")
##            print(parc_ant_posgba)
##            print("parc_ant_posgba_0")
##            print(parc_ant_posgba_0)
##
##            for i in range (len(manz_ant_wgs84)):  ##transforma las coordenadas de la manzana antecedente de wgs84 a posgar caba para poder comparar con la parcela del profesional
##                x,y = (manz_ant_wgs84[i])
##                print(x,y)
##                #manz_ant_posgba.append(transform(wgs84, posgba, x, y))
##                b,a = transformer.transform(y,x)
##                c = list()
##                c.append(a)
##                c.append(b)
##                manz_ant_posgba.append(c)
...
##                for i in range (len(ver_cep_p1)):
##                    diferencia_coord_3.append(diferencia_coord_2[i])
##                dif_max = round(max(diferencia_coord_3),2)
                ...
                if "-1" in band_tol_m:
                    validaciones2.loc[23,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
                    validaciones2.loc[23,'Observacion']="Error: La parcela medida cae fuera de la manzana a la cual pertenece (dentro de las tolerancias admitidas) o no se ha georreferenciado correctamente"
                    validaciones2.loc[23,'Cetegoría']='Tolerancias'

                    validaciones2.loc[24,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
                    validaciones2.loc[24,'Observacion']="Error: La parcela medida cae fuera de la manzana a la cual pertenece (dentro de las tolerancias admitidas) o no se ha georreferenciado correctamente"
                    validaciones2.loc[24,'Cetegoría']='Tolerancias'

                else:
                    validaciones2.loc[23,'Resultado']=0 # si hay un -1 en la bandera arroja error 
                    validaciones2.loc[23,'Observacion']="Ok: La parcela medida cae dentro de la manzana a la cual pertenece"
                    validaciones2.loc[23,'Cetegoría']='Tolerancias'
                    ...
                    if "-1" in band_tol_p:
                        ...
                        validaciones2.loc[24,'Observacion']="Verificar: La parcela medida excede la tolerancia admitida (es mas grande) respecto a la parcela antecedente o no se ha georreferenciado correctamente la parcela "
                        validaciones2.loc[24,'Cetegoría']='Tolerancias'

                    elif "-2" in band_tol_p:
                        
                        validaciones2.loc[24,'Resultado']=50 # si hay un -1 en la bandera arroja error 
                        validaciones2.loc[24,'Observacion']="Verificar: La parcela medida excede la tolerancia admitida (es mas chica) respecto a la parcela antecedente o no se ha georreferenciado correctamente la parcela"
                        validaciones2.loc[24,'Cetegoría']='Tolerancias'
                        ...
                        if dif_max > 0.50:
                            ...
                            validaciones2.loc[24,'Resultado']=50 # si hay un -1 en la bandera arroja error 
                            validaciones2.loc[24,'Observacion']="Verificar: La parcela medida excede la tolerancia admitida respecto a la parcela antecedente (0.5m)"
                            validaciones2.loc[24,'Cetegoría']='Tolerancias'

                        else:
                            
                            validaciones2.loc[24,'Resultado']=0 # si hay un -1 en la bandera arroja error 
                            validaciones2.loc[24,'Observacion']="Ok: La parcela medida se encuentra en tolerancia con respeto a la parcela antecedente"
                            validaciones2.loc[24,'Cetegoría']='Tolerancias'
            else:
                validaciones2.loc[23,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
        ...
        validaciones2.loc[24,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
        validaciones2.loc[24,'Observacion']="Error: Existen mas de un Layout de Ficha catastral con nomenclatura en su nombre SSS-MMMM-PPPP, con lo que no es posible validar las diferencias entre parcela medida y antecedente"
...
##
##    if len(validaciones2)>0:
##
##        pdf.cell(105,5, "Resumen Validación Archivo DXF",new_x=0,new_y=6,align='c')
##      pdf.cell(105,1, "Dirección General de Registro de Obras y Catastro", ln=True, border=False)
##      pdf.cell(105,1, "Gerencia Operativa de Catastro", ln=True, border=False)
##      pdf.cell(105,1, "Subgerencia Operativa de Registro de Mensuras", ln=True, border=False)
##        pdf.output("Resumen_validacion_DXF")
##
##    else:
##        
...
Boton_Chequear_layers.configure(font=("Gotham Rounded",10))
Boton_Chequear_layers.grid(padx=25, pady=5, row = 1, column=3)
