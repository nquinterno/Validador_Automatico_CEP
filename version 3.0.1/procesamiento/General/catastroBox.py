...
def midPointArc(arc):
    ...
    # Calcular ángulo total respetando sentido antihorario
    if end_angle < start_angle:
        end_angle += 360

    mid_angle = (start_angle + (end_angle - start_angle) / 2) % 360
    angle_rad = math.radians(mid_angle)

    x = centro[0] + radio * math.cos(angle_rad)
    y = centro[1] + radio * math.sin(angle_rad)
    ...
    if parcela.has_arc:
        ...
        for seg in segmentos:
            if seg.dxftype()== "ARC":
                ...
                if ezdxf.math.is_point_in_polygon_2d(Vec2(pto_arc),vertices_p3,abs_tol=1e-3)==-1:
                    if ezdxf.math.is_point_in_polygon_2d(Vec2(pto_arc),vertices_p4,abs_tol=1e-3)==-1:
...
def polig_dentro_polig(polig_madre, polig_hijo, tol):
    ...
    if type(polig_madre) != list:
        if polig_madre.has_arc:
            ...
            for vert in polig_madre.vertices():
                if vert in vertices_pm:
                    pass
                else:
                    vertices_pm.append(vert)
    ...
    if polig_hijo.has_arc:
        ...
        for vert in polig_hijo.vertices():
            if vert in vertices_ph:
                pass
            else:
                vertices_ph.append(vert)
...
def discretizar_curva(arc, tol):
    ...
    angle_0 = end_angle - start_angle  # siempre positivo (sentido antihorario)
    centro = arc.dxf.center
    radio = arc.dxf.radius

    # Buscar la mínima división del arco que cumpla la tolerancia
    arc_0 = arc
    flecha = 100
    ...
    while flecha > tol:
        ...
        midpoint_cuerda = (
            (arc_nuevo.start_point[0] + arc_nuevo.end_point[0]) / 2,
            (arc_nuevo.start_point[1] + arc_nuevo.end_point[1]) / 2
        )

        flecha = math.hypot(midpoint_cuerda[0] - midpoint_arc[0], midpoint_cuerda[1] - midpoint_arc[1])
        arc_0 = arc_nuevo

    angulo_ini_0 = abs(arc_0.end_angle - arc_0.start_angle)
    num_vert = max(1, int(angle_0 / angulo_ini_0))
    angulo_por_segmento_deg = angle_0 / num_vert
    angulo_por_segmento_rad = math.radians(angulo_por_segmento_deg)
    ...
    for i in range(num_vert + 1):
        ang = angulo_por_segmento_rad * i
        x = centro[0] + (dx * math.cos(ang) - dy * math.sin(ang))
        y = centro[1] + (dx * math.sin(ang) + dy * math.cos(ang))
        vert_arc.append([round(x, 3), round(y, 3)])
...
def invasiones(manzana, parcela, poligono ,tol):
    poligono_dizcretizado_0 = list()
    poligono_dizcretizado_1 = list()
    vertices_manzana = list()
    vertices_parcela = list()
    vertices_poligono = list()
    
    #prepara los poligonos madre e hijo para comparar, si tiene arco algun poligono genera vertices dentro del arco para que la flecha sea menor a 0.01m

    if type(manzana) != list:
        if manzana.has_arc:
            for lado in manzana.virtual_entities():
                if lado.dxftype()== "ARC":
                    arco_dizcretizado = discretizar_curva(lado,0.01) #lista de vertices de un arco para discretizarlo en lineas
                    poligono_dizcretizado_0 = poligono_dizcretizado_0 + arco_dizcretizado
                else:
            ...
            for vert in poligono_dizcretizado_0:
                if vert in vertices_manzana:
                    pass
                else:
                    vertices_manzana.append(vert)

        else:

            for vert in manzana.vertices() :
                if vert in vertices_manzana:
                    pass
                else:
                    vertices_manzana.append(vert)
    ...
    if type(parcela) != list:
        if parcela.has_arc:
            for lado in parcela.virtual_entities():
                if lado.dxftype()== "ARC":
                    arco_dizcretizado = discretizar_curva(lado,0.01) #lista de vertices de un arco para discretizarlo en lineas
                    poligono_dizcretizado_0 = poligono_dizcretizado_0 + arco_dizcretizado
                else:
            ...
            for vert in poligono_dizcretizado_0:
                if vert in vertices_parcela:
                    pass
                else:
                    vertices_parcela.append(vert)
    ...
    if poligono.has_arc:
        ...
        for lado in poligono.virtual_entities():
            if lado.dxftype()== "ARC":
                arco_dizcretizado = discretizar_curva(lado,0.01) #lista de vertices de un arco para discretizarlo en lineas
                poligono_dizcretizado_0 = poligono_dizcretizado_0 + arco_dizcretizado
            else:
        ...
        for vert in poligono_dizcretizado_0:
            if vert in vertices_poligono:
                pass
            else:
                vertices_poligono.append(vert)
    else:

        for vert in poligono.vertices():
            if vert in vertices_poligono:
                pass
            else:
                vertices_poligono.append(vert)
    ...
    # vertices_pm1 = Vec2.list(vertices_pm)
    # vertices_ph1 = Vec2.list(vertices_ph)
    vertices_manzana_1 = list()
    vertices_parcela_1 = list()
    vertices_poligono_1 = list()


    for vert in vertices_manzana:
        vertices_manzana_1.append(Vec2(vert[0],vert[1]))

    for vert in vertices_parcela:
        vertices_parcela_1.append(Vec2(vert[0],vert[1]))


    for vert in vertices_poligono:
        vertices_poligono_1.append(Vec2(vert[0],vert[1]))
    
    #vertices_p1 = parcela.get_points('xy')  #para cada parcela obtiene los vertices y los prepara para la función de control

    vertices_manzana_2 = list(ezdxf.math.offset_vertices_2d(vertices_manzana_1,offset=tol*(-1), closed=True))
    vertices_manzana_3 = list(ezdxf.math.offset_vertices_2d(vertices_manzana_1,offset=tol, closed=True))

    vertices_parcela_2 = list(ezdxf.math.offset_vertices_2d(vertices_parcela_1,offset=tol*(-1), closed=True))
    vertices_parcela_3 = list(ezdxf.math.offset_vertices_2d(vertices_parcela_1,offset=tol, closed=True))
    band_vert_invasion = list()

    for i in range (0,len(vertices_poligono_1)): 
        
        if ezdxf.math.is_point_in_polygon_2d(vertices_poligono_1[i],vertices_manzana_2,abs_tol=1e-3)==-1: #validación de vertices arroja -1 si cae fuera, 0 si cae en los limites y 1 si cae dentro
            if ezdxf.math.is_point_in_polygon_2d(vertices_poligono_1[i],vertices_manzana_3,abs_tol=1e-3)==-1:
                band_vert_dentro_manz='-1'
            else:
                band_vert_dentro_manz='0'
        else:
            band_vert_dentro_manz='0'

        if ezdxf.math.is_point_in_polygon_2d(vertices_poligono_1[i],vertices_parcela_2,abs_tol=1e-3)==-1: #validación de vertices arroja -1 si cae fuera, 0 si cae en los limites y 1 si cae dentro
            if ezdxf.math.is_point_in_polygon_2d(vertices_poligono_1[i],vertices_parcela_3,abs_tol=1e-3)==-1:
                band_vert_dentro_parc='-1'
            else:
                band_vert_dentro_parc='0'
        else:
            band_vert_dentro_parc='0'

        if  band_vert_dentro_manz == '-1' and band_vert_dentro_parc=='-1':
            band_vert_invasion.append('invasion')
        elif band_vert_dentro_manz == '0' and band_vert_dentro_parc=='-1':
            band_vert_invasion.append('encaballada')
        elif band_vert_dentro_manz == '0' and band_vert_dentro_parc=='0': 
            band_vert_invasion.append('-')
        elif band_vert_dentro_manz == '-1' and band_vert_dentro_parc=='0': 
            band_vert_invasion.append('invasion')
        else:
            pass
  
    if 'invasion' in band_vert_invasion and "encaballada" in band_vert_invasion:
        resultado = "invasion y encaballada"
    elif 'invasion' in band_vert_invasion:
        resultado = "invasion"
    elif "encaballada" in band_vert_invasion:
        resultado = "encaballada"
    else:
        resultado = "ok"
    
    return resultado

# def parcelas_contiguas(polig_madre, polig_hijo, tol):
...
#     vertices_pm4 = #tengo que tomar el poligono mas grande de los dos offsets
    
#     # convertir listas de vértices en polígonos
#     poly2 = Polygon(vertices_pm2)
#     poly3 = Polygon(vertices_pm3)
...
#     if area3 > area2:
#         vertices_pm4 = vertices_pm3
#     else:
#         vertices_pm4 = vertices_pm2
...
#         if ezdxf.math.is_point_in_polygon_2d(vertices_ph1[i],vertices_pm1,abs_tol=1e-3)==-1:
#              #validación de vertices arroja -1 si cae fuera, 0 si cae en los limites y 1 si cae dentro
#             if ezdxf.math.is_point_in_polygon_2d(vertices_ph1[i],vertices_pm4,abs_tol=1e-3)==-1:
#                 band_vert_dentro.append('-1')
#             else:
#                 band_vert_dentro.append('0')
#         else:
