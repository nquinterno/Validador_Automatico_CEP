...
##    Calcular la cantidad de lados que tiene la parcela
    ...
    if len(parcelas): #si hay entindades en el layer patrcela comienza la validacion de las mismas sino arroja error que no hay nada en ese layer
        ...
        if "-1" in band_poly_parc: # si hay un -1 en la bandera arroja error 
            ...
            if len(parcelas_poly)>0:
                ...
                for parcela in parcelas_poly:
                    ...
                    medidas_parcelas.iloc[clave] = medidas_dxf
        ...
        if color_parc == "0" and gro_parc == "0":
            ...
            validaciones_model.loc[2,'Resultado']=-1 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
    ...
    if len(excedentes): #si hay entindades en el layer patrcela comienza la validacion de las mismas sino arroja error que no hay nada en ese layer
        ...
        if color_exc == "0" and gro_exc == "0":
            ...
            validaciones_model.loc[7,'Resultado']=-1 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
    ...
    if len(bloque_PARCELA_SURGENTE)==len(parcelas_poly):
            ...
            validaciones_model.loc[9,'Observacion']='OK: Exite inserto un bloque "PARCELA_SURGENTE" por cada parcela'
            ...
            for bloque in (bloque_PARCELA_SURGENTE):
                ...
                for parcela in parcelas_poly:
                    ...
                    if ezdxf.math.is_point_in_polygon_2d(vertice_nomenc,vertices_mp1,abs_tol=1e-3)==-1:
                        ...
                        parcelas_dict.iloc[nomen_parc] = parcela
            


    elif len(bloque_PARCELA_SURGENTE)>len(parcelas_poly):
            validaciones_model.loc[9,'Resultado']=-2
            validaciones_model.loc[9,'Observacion']='ERROR: Exite más de un boque "PARCELA_SURGENTE" insertos que parcelas'
            validaciones_model.loc[9,'Cetegoría']='Bloques'
 ...
 #valida que haya la misma cantidad de bloques de parcela insertos en el dxf que la cantidad de parcelas del layer parcelas
    ...
    if "-1" in band_layer_sup:
        ...
        validaciones_model.loc[16,'Observacion']="Error: Alguno de los Layers de Superficies no se ha creado con el nombre correcto"
        validaciones_model.loc[16,'Cetegoría']='Mejoras'
    else:
        validaciones_model.loc[16,'Resultado']=0
        validaciones_model.loc[16,'Observacion']="Ok: Los layers de superficies poseen el formato correcto en el nombre"
        validaciones_model.loc[16,'Cetegoría']='Mejoras'
    ...
    if len(mejoras): #si hay entindades en el layer mejoras comienza la validacion de las mismas sino arroja error que no hay nada en ese layer
        ...
        if "-1" in band_poly_mej: # si hay un -1 en la bandera arroja error 
            validaciones_model.loc[17,'Resultado']=-1
            ...
            validaciones_model.loc[20,'Observacion']='Error: Existen entidades distintas de Polylineas en alguno de los layers de "-SUP"'
            validaciones_model.loc[20,'Cetegoría']='Mejoras'
        ...
        validaciones_model.loc[18,'Observacion']="OK: No corresponde polig. de superficies puesto que no se han dibujado en los layers '-SUP', verifique que se trate de un CEP Demolición"        
        validaciones_model.loc[18,'Cetegoría']='Mejoras'

        validaciones_model.loc[19,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones_model.loc[19,'Observacion']="OK: No corresponde polig. de superficies puesto que no se han dibujado en los layers '-SUP', verifique que se trate de un CEP Demolición"
        validaciones_model.loc[19,'Cetegoría']='Mejoras'

        validaciones_model.loc[20,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones_model.loc[20,'Observacion']="OK: No corresponde polig. de superficies puesto que no se han dibujado en los layers '-SUP', verifique que se trate de un CEP Demolición"        
        validaciones_model.loc[20,'Cetegoría']='Mejoras'
