...
def chequeo_model_mens(doc,nom_layers_dxf,colores):
    ...
    cesiones = doc.modelspace().query('*[layer=="12-M-CESION"]')
    ...
    parcelas_dict = dict() #diccionario que guarda las parcelas mensuradas con su nomenclatura como clave y la entidad poligono como valor
    medidas_parcelas = dict()
    
##    Calcular la cantidad de lados que tiene la parcela

    if len(parcelas): #si hay entindades en el layer parcela comienza la validacion de las mismas sino arroja error que no hay nada en ese layer
        ...
        if len(bloque_PARCELA_SURGENTE)==len(parcelas_poly):
            validaciones_model.loc[0,'Resultado']=0
            validaciones_model.loc[0,'Observacion']='OK: Exite inserto un bloque "PARCELA_SURGENTE" por cada parcela'
            validaciones_model.loc[0,'Cetegoría']='Bloques'
                ...
                for attrib in bloque.attribs:
                    if attrib.dxf.tag=="NPARC":
                        ...
                        if attrib.dxf.text == None:
                            ...
                            nomen_parc = attrib.dxf.text
                    else:
                        ban_bparc_comp.append(-2)
                        nom_parc_list.append("0")

                if -1 in ban_bparc_comp:
                    validaciones_model.loc[1,'Resultado']=-1
                    validaciones_model.loc[1,'Observacion']='ERROR: El o los bloques "PARCELA_SURGENTE" estan vacíos'
                    validaciones_model.loc[1,'Cetegoría']='Bloques' 
                elif -2 in ban_bparc_comp:
                    validaciones_model.loc[1,'Resultado']=-1
                    validaciones_model.loc[1,'Observacion']='ERROR: Se modificó el bloque PARCELA_SURGENTE, utilice el de la plantilla.'
                    validaciones_model.loc[1,'Cetegoría']='Bloques' 
                    ...
                    if None in band_nom_parc:
                        validaciones_model.loc[1,'Resultado']=-1
                        validaciones_model.loc[1,'Observacion']='ERROR: La nomenclatura indicada en los bloque "PARCELA_SURGENTE" no respeta el formato 000a'
                        validaciones_model.loc[1,'Cetegoría']='Bloques'
                    else:
                        validaciones_model.loc[1,'Resultado']=0
                        validaciones_model.loc[1,'Observacion']='OK: La nomenclatura indicada los bloque "PARCELA_SURGENTE" respeta el formato 000a'
                        validaciones_model.loc[1,'Cetegoría']='Bloques'

                vertice_nomenc = Vec2(bloque.dxf.insert.x, bloque.dxf.insert.y)
                for parcela in parcelas_poly:
                    vertices_mp0 = parcela.get_points('xy')  #para cada parcela obtiene los vertices y los prepara para la función de control
                    vertices_mp1 = Vec2.list(vertices_mp0)

                    if ezdxf.math.is_point_in_polygon_2d(vertice_nomenc,vertices_mp1,abs_tol=1e-3)==-1:
                        pass
                    else:
                        parcelas_dict[nomen_parc] = parcela
            
            if len(parcelas_dict) == len(parcelas_poly):
            
                band_nom_parc = "0" #bandera que detecta si todas las parcelas tienen un bloque de nomenclatura dentro
            else:
                band_nom_parc = "-1"
        
        elif len(bloque_PARCELA_SURGENTE)>len(parcelas_poly):
                validaciones_model.loc[0,'Resultado']=-2
                validaciones_model.loc[0,'Observacion']='ERROR: Exite más de un boque "PARCELA_SURGENTE" insertos que parcelas'
                validaciones_model.loc[0,'Cetegoría']='Bloques'

                validaciones_model.loc[1,'Resultado']=-1
                validaciones_model.loc[1,'Observacion']='ERROR: Exite más de un boque "PARCELA_SURGENTE" inserto'
                validaciones_model.loc[1,'Cetegoría']='Bloques'
                ...
                validaciones_model.loc[0,'Observacion']='ERROR: No coincide la cantidad de bloques "PARCELA_SURGENTE" con la cantidad de parcelas'
                validaciones_model.loc[0,'Cetegoría']='Bloques'

                validaciones_model.loc[1,'Resultado']=-1
                validaciones_model.loc[1,'Observacion']='ERROR: No coincide la cantidad de bloques "PARCELA_SURGENTE" con la cantidad de parcelas'
                validaciones_model.loc[1,'Cetegoría']='Bloques'
    ...
    #valida que haya la misma cantidad de bloques de parcela insertos en el dxf que la cantidad de parcelas del layer parcelas
        ...
        if "-1" in band_poly_parc: # si hay un -1 en la bandera arroja error 
            validaciones_model.loc[2,'Resultado']=-1
            validaciones_model.loc[2,'Observacion']="ERROR: Existen entidades distintas de Polylineas y Bloque de nomenclatura en el Layer 09-M-PARCELA"
            ...
            if '-1' in band_poly_cer_parc:
                validaciones_model.loc[2,'Resultado']=-2 # si hay un -1 en la bandera arroja error 
                validaciones_model.loc[2,'Observacion']="ERROR: Se dibujaron polylineas Abiertas en el layer 09-M-PARCELA"
                validaciones_model.loc[2,'Cetegoría']='Parcela/s'  

            else:# si no hay un -1 en la bandera sigue validando color y grosor de polylineas.

                validaciones_model.loc[2,'Resultado']=0 # si hay un -1 en la bandera arroja error 
                validaciones_model.loc[2,'Observacion']="OK: Se dibujaron polylineas Cerradass en el layer 09-M-PARCELA"
                validaciones_model.loc[2,'Cetegoría']='Parcela/s'

        if len(parcelas_dict)>0:


            for clave, parcela in parcelas_dict.items():
                ...
                medidas_parcelas[clave] = medidas_dxf
        ...
        if len(parcelas_poly) == 0:
            validaciones_model.loc[3,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
            validaciones_model.loc[3,'Observacion']="ERROR: No se dibujaron polylineas en el layer 09-M-PARCELA"
            validaciones_model.loc[3,'Cetegoría']='Parcela/s'    

        else:
            validaciones_model.loc[3,'Resultado']=0 # si hay un -1 en la bandera arroja error 
            validaciones_model.loc[3,'Observacion']="OK: Se dibujó polylinea en el layer 09-M-PARCELA"
            validaciones_model.loc[3,'Cetegoría']='Parcela/s'
        ...
        if "-1" in band_poly_color_parc:
            color_parc = "-1"
        else:
            color_parc = "0"
        if "-1" in band_poly_gro_parc:
            gro_parc = "-1"
        else:
            gro_parc = "0"
        
        if color_parc == "0" and gro_parc == "0":
            validaciones_model.loc[4,'Resultado']=0 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
            validaciones_model.loc[4,'Observacion']="OK: Se dibujaron polylineas Cerradas configuradas correctamente en el layer 09-M-PARCELA"
            validaciones_model.loc[4,'Cetegoría']='Parcela/s'  

        elif color_parc == "0" and gro_parc == "-1":
            validaciones_model.loc[4,'Resultado']=-1 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
            validaciones_model.loc[4,'Observacion']="ERROR: Grosor de polylineas erroneo en layer 09-M-PARCELA debe ser 'Default'"
            validaciones_model.loc[4,'Cetegoría']='Parcela/s' 

        elif color_parc == "-1" and gro_parc == "0":
            validaciones_model.loc[4,'Resultado']=-1 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
            validaciones_model.loc[4,'Observacion']="ERROR: Color de polylineas erroneo en layer 09-M-PARCELA debe ser Azul"
            validaciones_model.loc[4,'Cetegoría']='Parcela/s'  

        elif color_parc == "-1" and gro_parc == "-1":
            validaciones_model.loc[4,'Resultado']=-1 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
            validaciones_model.loc[4,'Observacion']="ERROR: Color y grosor de polylineas erroneo en layer 09-M-PARCELA"
            validaciones_model.loc[4,'Cetegoría']='Parcela/s'
        ...
        validaciones_model.loc[3,'Resultado']=-1 #si no hay entindades en el layer parcela arroja error de validación
        validaciones_model.loc[3,'Observacion']="ERROR: No se ecuentra diibujada la PARCELA en el Layer 09-M-PARCELA"
        validaciones_model.loc[3,'Cetegoría']='Parcela/s'

        validaciones_model.loc[4,'Resultado']=-1 #si no hay entindades en el layer parcela arroja error de validación
        validaciones_model.loc[4,'Observacion']="ERROR: No se ecuentra diibujada la PARCELA en el Layer 09-M-PARCELA"
        validaciones_model.loc[4,'Cetegoría']='Parcela/s'
    ...
    if len(m_manzana)>0:
        for element in m_manzana:
            ...
            validaciones_model.loc[5,'Observacion']="ERROR: Verifique que haya pegado con coordenadas originales los elementos del dxf de la manzana correspondiente en el layer 07-M-MANZANA"
            validaciones_model.loc[5,'Cetegoría']='Antecedente'

    if "0" in band_ant_manzana:
            validaciones_model.loc[5,'Resultado']=0
            validaciones_model.loc[5,'Observacion']="OK: El layer 07-M-MANZANA no se encuentra vacío"
            validaciones_model.loc[5,'Cetegoría']='Antecedente'  
    ...
    if len(m_calles)>0:
        ...
        if "-1" in band_calles_text:
            ...
            validaciones_model.loc[6,'Observacion']="ERROR:En el layer 05-M-NOMBRES-DE-CALLE deben indicarse los nombres de calle del DXF de la manzana correspondiente con entidad 'TEXT'"
            validaciones_model.loc[6,'Cetegoría']='Antecedente'

        else:
            validaciones_model.loc[6,'Resultado']=0
            validaciones_model.loc[6,'Observacion']="OK: En el layer 05-M-NOMBRES-DE-CALLE se encuentran indicadas solo entidades de 'TEXT'"
            validaciones_model.loc[6,'Cetegoría']='Antecedente'
                
    else:
        validaciones_model.loc[6,'Resultado']=-1
        validaciones_model.loc[6,'Observacion']="ERROR: El layer 05-M-NOMBRES-DE-CALLE está vacío, debe copiarse con coord. originales los elementos del DXF de la manzana correspondiente"
        validaciones_model.loc[6,'Cetegoría']='Antecedente'

    if "-1" in band_num_layer:
        validaciones_model.loc[7,'Resultado']=-1
        validaciones_model.loc[7,'Observacion']="ERROR: Existen N° de puerta en layer distinto a '06-M-NUMERO-DE-PUERTA'"
        validaciones_model.loc[7,'Cetegoría']='Antecedente'
    else:
        validaciones_model.loc[7,'Resultado']=0
        validaciones_model.loc[7,'Observacion']="Ok: Se indicaron los N° de puerta en el layer correspondiente"
        validaciones_model.loc[7,'Cetegoría']='Antecedente'
    ...
    if len(excedentes): #si hay entindades en el layer patrcela comienza la validacion de las mismas sino arroja error que no hay nada en ese layer
        ...
        if "-1" in band_poly_exc: # si hay un -1 en la bandera arroja error 
            validaciones_model.loc[8,'Resultado']=-1
            validaciones_model.loc[8,'Observacion']="ERROR: Existen entidades distintas de Polylineas en el Layer EXCEDENTE"
            ...
            if '-1' in band_poly_cer_exc:
                    validaciones_model.loc[8,'Resultado']=-2 # si hay un -1 en la bandera arroja error 
                    validaciones_model.loc[8,'Observacion']="ERROR: Se dibujaron polylineas Abiertas en el layer EXCEDENTE"
                    validaciones_model.loc[8,'Cetegoría']='Parcela/s'
            else:
                validaciones_model.loc[8,'Resultado']=0 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
                validaciones_model.loc[8,'Observacion']="OK: Se dibujaron polylineas Cerradas en el layer EXCEDENTE"
                validaciones_model.loc[8,'Cetegoría']='Parcela/s'#si la polylinea es cerrada sigue validando que tengan el color y grosor bylayer
        ...
        for excedente in excedentes: 
            ...
            print("linea excedente: ",excedente.dxf.lineweight)
        ...
        if "-1" in band_poly_color_exc:
            color_exc = "-1"
        else:
            color_exc = "0"
        
        if "-1" in band_poly_gro_exc:
            gro_exc = "-1"
        else:
            gro_exc = "0"

        if color_exc == "0" and gro_exc == "0":
            validaciones_model.loc[9,'Resultado']=0 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
            validaciones_model.loc[9,'Observacion']="OK: Se dibujaron polylineas Cerradas configuradas correctamente en el layer EXCEDENTE"
            validaciones_model.loc[9,'Cetegoría']='Parcela/s'
            
        elif color_exc == "0" and gro_exc == "-1":
            validaciones_model.loc[9,'Resultado']=-1 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
            validaciones_model.loc[9,'Observacion']="ERROR: Grosor de polylineas erroneo en layer EXCEDENTE"
            validaciones_model.loc[9,'Cetegoría']='Parcela/s'
            
        elif color_exc == "-1" and gro_exc == "0":
            validaciones_model.loc[9,'Resultado']=-1 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
            validaciones_model.loc[9,'Observacion']="ERROR: Color de polylineas erroneo en layer EXCEDENTE"
            validaciones_model.loc[9,'Cetegoría']='Parcela/s'
            
        elif color_exc == "-1" and gro_exc == "-1":
            validaciones_model.loc[9,'Resultado']=-1 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
            validaciones_model.loc[9,'Observacion']="ERROR: Color y grosor de polylineas erroneo en layer EXCEDENTE"
            validaciones_model.loc[9,'Cetegoría']='Parcela/s'
        ...
        if '-1' in band_exc_dentro:
            validaciones_model.loc[10,'Resultado']=-1 # Valida si las polilineas del layer parcela tienen el grosor y color blayer
            validaciones_model.loc[10,'Observacion']="ERROR: Existe al menos un excedente fuera de la o las Parcelas mensuradas"
            validaciones_model.loc[10,'Cetegoría']='Parcela/s'
        else:
            validaciones_model.loc[10,'Resultado']=0 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
            validaciones_model.loc[10,'Observacion']="OK: El o los excedentes se encuentran completamente dentro de la/las Parcelas Mensuradas"
            validaciones_model.loc[10,'Cetegoría']='Parcela/s'

            ## FIN VERIFICACIÓN EXCEDENTE ESTE COMPLETAMENTE DENTRO DEL POLIGONO DE PARCELA.
    else:
        
        validaciones_model.loc[8,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
        ...
        validaciones_model.loc[9,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones_model.loc[9,'Observacion']="OK: Validación de excedente no corresponde por encontrase el layer vacío, verifique que no exista uno para la parcela medida"
        validaciones_model.loc[9,'Cetegoría']='Parcela/s'

        validaciones_model.loc[10,'Resultado']=99 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
        validaciones_model.loc[10,'Observacion']="OK: Validación de excedente no corresponde por encontrase el layer vacío, verifique que no exista uno para la parcela medida"
        validaciones_model.loc[10,'Cetegoría']='Parcela/s'
     
    #--- 2.5.2 FIN Validación de Excedente --#


    # Inicio Validacion Cesiones  
    band_poly_ces = list()
    band_poly_ces_close = list()
    cesion_poly_close = list()
    band_cesion_color = list()
    band_cesion_gro = list()
    band_ces_dentro = list()
    ...
    for cesion in cesiones:
        if cesion.dxftype()!= "LWPOLYLINE": # agrega -1 a la bandera cuando la entidas no es lwpolyline
            ...
            band_poly_ces.append('0')
            if cesion.closed:
                cesion_poly_close.append(cesion)
                band_poly_ces_close.append('0')  # agrega 0 a la bandera cuando la polylinea es cerrada
            else:
                band_poly_ces_close.append('-1')
        
        if cesion.dxf.color==colores.at[0,'Cesion']:
            band_cesion_color.append('0')
        else:
            band_cesion_color.append('-1')
        if cesion.dxf.lineweight==-1:
            band_cesion_gro.append('0') 
        else:
            band_cesion_gro.append('-1')

        for parcela in parcelas_poly:
            if polig_dentro_polig(parcela, cesion, 0.01):
                band_ces_dentro.append('-1')
            else:
                band_ces_dentro.append('0')

    if "-1" in band_cesion_color:
        cesion_color = "-1"
    else:
        cesion_color = "0"
    if "-1" in band_cesion_gro:
        cesion_gro = "-1"
    else:
        cesion_gro = "0"
    ...
    if "-1" in band_poly_ces: # si hay un -1 en la bandera arroja error 
        validaciones_model.loc[11,'Resultado']=-1
        validaciones_model.loc[11,'Observacion']="ERROR: Existen entidades distintas de Polylineas en el Layer CESION"
        validaciones_model.loc[11,'Cetegoría']='Cesiones'
    else:
        validaciones_model.loc[11,'Resultado']=0
        validaciones_model.loc[11,'Observacion']="OK: No existen entidades distintas de Polylineas en el Layer CESION"
        validaciones_model.loc[11,'Cetegoría']='Cesiones'

    if '-1' in band_poly_ces_close:
        validaciones_model.loc[12,'Resultado']=-1 
        validaciones_model.loc[12,'Observacion']="ERROR: Se dibujaron polylineas Abiertas en el layer CESION"
        validaciones_model.loc[12,'Cetegoría']='Cesiones'
    else:
        validaciones_model.loc[12,'Resultado']=0
        validaciones_model.loc[12,'Observacion']="OK: Se dibujaron polylineas Cerradas en el layer CESION"
        validaciones_model.loc[12,'Cetegoría']='Cesiones'
    
    if cesion_color == "0" and cesion_gro == '0':
        validaciones_model.loc[13,'Resultado']=0 # Valida si las polilineas del layer cesion tienen el grosor y color bylayer
        validaciones_model.loc[13,'Observacion']="OK: Se dibujaron polilineas Cerradas configuradas correctamente en el layer CESION"
        validaciones_model.loc[13,'Cetegoría']='Cesiones'
    
    elif cesion_color == "0" and cesion_gro == '-1':
        validaciones_model.loc[13,'Resultado']=-1 # Valida si las polilineas del layer cesion tienen el grosor y color bylayer
        validaciones_model.loc[13,'Observacion']="ERROR: Grosor de polilineas erroneo en layer CESION"
        validaciones_model.loc[13,'Cetegoría']='Cesiones'
    
    elif cesion_color == "-1" and cesion_gro == '0':
        validaciones_model.loc[13,'Resultado']=-1 # Valida si las polilineas del layer cesion tienen color rojo
        validaciones_model.loc[13,'Observacion']="ERROR: Color de polilineas erroneo en layer CESION"
        validaciones_model.loc[13,'Cetegoría']='Cesiones'

    elif cesion_color == "-1" and cesion_gro == '-1':
        validaciones_model.loc[13,'Resultado']=-1 # Valida si las polilineas del layer cesion tienen el grosor y color bylayer
        validaciones_model.loc[13,'Observacion']="ERROR: Color y grosor de polilineas erroneo en layer CESION"
        validaciones_model.loc[13,'Cetegoría']='Cesiones'

    if '-1' in band_ces_dentro:
        validaciones_model.loc[14,'Resultado']=-1 
        validaciones_model.loc[14,'Observacion']="ERROR: Existe al menos una cesión dentro de la o las Parcelas mensuradas"
        validaciones_model.loc[14,'Cetegoría']='Cesiones'
    else:
        validaciones_model.loc[14,'Resultado']=0
        validaciones_model.loc[14,'Observacion']="OK: Las cesiones se encuentran fuera de la/las Parcelas Mensuradas"
        validaciones_model.loc[14,'Cetegoría']='Cesiones'


    #FIn Validadion Cesiones
    ...
    #     validaciones_model.loc[14,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
    #     validaciones_model.loc[14,'Observacion']="Error: Algun bloque de formulario no se encuentra inserto dentro del poligono de superficie al que pertenece"
    #     validaciones_model.loc[14,'Cetegoría']='Mejoras'
        
        
    # else:
    #     validaciones_model.loc[14,'Resultado']=0 
    #     validaciones_model.loc[14,'Observacion']="Ok: Los bloques de formularios se encuentran insertos dentro de los poligono de superficie a los que pertenecen"
    #     validaciones_model.loc[14,'Cetegoría']='Mejoras'
    ...
    #     validaciones_model.loc[15,'Resultado']=-1 # si hay un -1 en la bandera arroja error 
    #     validaciones_model.loc[15,'Observacion']="Error: En alguno de los pisos difiere la cantidad de polilineas cerradas que bloques de formulario insertos"
    #     validaciones_model.loc[15,'Cetegoría']='Mejoras'
        
    # else:
    #     validaciones_model.loc[15,'Resultado']=0 # si hay un -1 en la bandera arroja error 
    #     validaciones_model.loc[15,'Observacion']="Ok: En todos los pisos existe la misma cantidad de polilineas cerradas de superficies que bloques de formulario insertos"
    #     validaciones_model.loc[15,'Cetegoría']='Mejoras'
    ...
    if "-1" in band_layer_sup:
        validaciones_model.loc[15,'Resultado']=-1
        validaciones_model.loc[15,'Observacion']="Error: Alguno de los Layers de Superficies no se ha creado con el nombre correcto"
        validaciones_model.loc[15,'Cetegoría']='Mejoras'
    else:
        validaciones_model.loc[15,'Resultado']=0
        validaciones_model.loc[15,'Observacion']="Ok: Los layers de superficies poseen el formato correcto en el nombre"
        validaciones_model.loc[15,'Cetegoría']='Mejoras'
    ...
    if len(mejoras): #si hay entindades en el layer mejoras comienza la validacion de las mismas sino arroja error que no hay nada en ese layer
        ...
        if "-1" in band_poly_mej: # si hay un -1 en la bandera arroja error 
            validaciones_model.loc[16,'Resultado']=-1
            validaciones_model.loc[16,'Observacion']='Error: Existen entidades distintas de Polylineas en alguno de los layers de "-SUP"'
            ...
            validaciones_model.loc[17,'Observacion']='Error: Existen entidades distintas de Polylineas en alguno de los layers de  "-SUP"'
            ...
            validaciones_model.loc[18,'Observacion']='Error: Existen entidades distintas de Polylineas en alguno de los layers de "-SUP"'
            validaciones_model.loc[18,'Cetegoría']='Mejoras'

            validaciones_model.loc[19,'Resultado']=-1 # si no hay un -1 en la bandera indica que la validación es correcta.
            validaciones_model.loc[19,'Observacion']='Error: Existen entidades distintas de Polylineas en alguno de los layers de "-SUP"'
            validaciones_model.loc[19,'Cetegoría']='Mejoras'
            
            resultado_inv = "ok"
        else:
            validaciones_model.loc[16,'Resultado']=0
            validaciones_model.loc[16,'Observacion']='OK: NO existen entidades distintas de Polylineas en alguno de los layers de "-SUP"'
            validaciones_model.loc[16,'Cetegoría']='Mejoras'           
            ...
            if '-1' in band_poly_cer_mej:
                validaciones_model.loc[17,'Resultado']=-2 # si hay un -1 en la bandera arroja error 
                validaciones_model.loc[17,'Observacion']="Error: Se dibujaron polylineas Abiertas en alguno de los layers de '-SUP', deben ser Cerradas"
                validaciones_model.loc[17,'Cetegoría']='Mejoras'
            else:
                validaciones_model.loc[17,'Resultado']=0 # si hay un -1 en la bandera arroja error 
                validaciones_model.loc[17,'Observacion']="OK: Se dibujaron polylineas Cerradas en los layers de '-SUP'"
                validaciones_model.loc[17,'Cetegoría']='Mejoras'
            ...
            if '-1' in band_poly_color_mej:
                validaciones_model.loc[18,'Resultado']=-1 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
                validaciones_model.loc[18,'Observacion']="Error: Color de polylineas erroneo en los layers de '-SUP' (colores admitidos rojo, amarillo, verde, cian, azul)"
                validaciones_model.loc[18,'Cetegoría']='Mejoras'
            
            else:
                validaciones_model.loc[18,'Resultado']=0 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
                validaciones_model.loc[18,'Observacion']="OK: Se dibujaron polylineas Cerradas configuradas correctamente en los layers de '-SUP'"
                validaciones_model.loc[18,'Cetegoría']='Mejoras'
                

            # if len(mejoras_poly) == (len(forms) + len(forms_empadronados) + len(forms_descubiertos) + len(forms_vacios)):
            #     validaciones_model.loc[19,'Resultado']=0 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
            #     validaciones_model.loc[19,'Observacion']="OK: Coinciden la cantidad de polilineas cerradas con la de bloques de formularios"
            #     validaciones_model.loc[19,'Cetegoría']='Mejoras'

            # else:
            #     validaciones_model.loc[19,'Resultado']=-1 # Valida si las polilineas del layer parcela tienen el grosor y color bylayer
            #     validaciones_model.loc[19,'Observacion']="ERROR: No Coinciden la cantidad de polilineas cerradas con la de bloques de formularios (form / form_emparonado / form_descubierto)"
            #     validaciones_model.loc[19,'Cetegoría']='Mejoras'
            ...
            if len(mejoras_poly_PB_SUB)>0:
                for mejora in mejoras_poly_PB_SUB: #reccorre poligonos de mejoras
                    band_mej_dentro_0 = list() #inicia bandera de control si cada uno de las mejoras estan dentro de una parcela
                    for parcela in parcelas_poly: #reccorre poligonos de parcelas
                        if polig_dentro_polig(parcela,mejora,0.01):
                            band_mej_dentro_0.append('0')
                        else:
                            band_mej_dentro.append('-1')
                    if "0" in band_mej_dentro_0:
                        band_mej_dentro.append("0") #si hay un 0 en la bandera indica que la mejora esta dentro de una parcela
                    else:
                        band_mej_dentro.append("-1")
                ...
                resultado_inv = "ok"
                band_mej_dentro.append("-2") #si no hay mejoras en PB o SUB arroja error

            if "-2" in band_mej_dentro: # si no hay mejoras en PB o SUB arroja error
                validaciones_model.loc[19,'Resultado']=99
                validaciones_model.loc[19,'Observacion']="OK: No se han dibujado mejoras en los layers de PB o SUB"
                validaciones_model.loc[19,'Cetegoría']='Mejoras'
            
            elif "-1" in band_mej_dentro: # si hay un -1 en la bandera arroja error
                validaciones_model.loc[19,'Resultado']=-1
                validaciones_model.loc[19,'Observacion']="Error: Alguna de las mejoras no se encuentra completamente dentro del poligono de parcela al que pertenece"
                validaciones_model.loc[19,'Cetegoría']='Mejoras'
            else:
                validaciones_model.loc[19,'Resultado']=0 # si no hay un -1 en la bandera indica que la validación es correcta.
                validaciones_model.loc[19,'Observacion']="OK: Todas las mejoras se encuentran completamente dentro del poligono de parcela al que pertenecen"
                validaciones_model.loc[19,'Cetegoría']='Mejoras'
        ...
        validaciones_model.loc[16,'Observacion']="OK: No corresponde validar polig. de superficies puesto que no se han dibujado en los layers '-SUP'"
        validaciones_model.loc[16,'Cetegoría']='Mejoras'

        validaciones_model.loc[17,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones_model.loc[17,'Observacion']="OK: No corresponde  validar polig. de superficies puesto que no se han dibujado en los layers '-SUP'"   
        validaciones_model.loc[17,'Cetegoría']='Mejoras'

        validaciones_model.loc[18,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones_model.loc[18,'Observacion']="OK: No corresponde validar polig. de superficies puesto que no se han dibujado en los layers '-SUP'"
        validaciones_model.loc[18,'Cetegoría']='Mejoras'

        validaciones_model.loc[19,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
        validaciones_model.loc[19,'Observacion']="OK: No corresponde validar polig. de superficies puesto que no se han dibujado en los layers '-SUP'"       
        validaciones_model.loc[19,'Cetegoría']='Mejoras'
    ...
    #         validaciones_model.loc[21,'Resultado']=-1 # si no hay un -1 en la bandera indica que la validación es correcta.
    #         validaciones_model.loc[21,'Observacion']="Error: En el bloque form no se ha completado algun campo"
    #         validaciones_model.loc[21,'Cetegoría']='Formularios'

    #     else:
    #         validaciones_model.loc[21,'Resultado']=0 # si no hay un -1 en la bandera indica que la validación es correcta.
    #         validaciones_model.loc[21,'Observacion']="OK: Se han completado los campos del bloque form"
    #         validaciones_model.loc[21,'Cetegoría']='Formularios'

    #     if "-1" in band_letra_form:
    #         validaciones_model.loc[22,'Resultado']=-1
    #         validaciones_model.loc[22,'Observacion']="Error: Los campos 'Nº_POLIG.','TIPO_FORM', y 'Nº_FORM' deben completarse unicamente con números enteros"
    #         validaciones_model.loc[22,'Cetegoría']='Mejoras'
    #     else:
    #         validaciones_model.loc[22,'Resultado']=0
    #         validaciones_model.loc[22,'Observacion']="Ok: Los campos 'Nº_POLIG.','TIPO_FORM', y 'Nº_FORM' se completaron unicamente con números entero"
    #         validaciones_model.loc[22,'Cetegoría']='Mejoras'
    ...
    #             validaciones_model.loc[23,'Resultado']=-1 # si no hay un -1 en la bandera indica que la validación es correcta.
    #             validaciones_model.loc[23,'Observacion']="Error: El primer Poligono indicado en el bloque form debe ser el Nº '1'"
    #             validaciones_model.loc[23,'Cetegoría']='Formularios'

    #         else:
    #             if "-1" in band_correl_polig:
    #                 validaciones_model.loc[23,'Resultado']=-1 # si no hay un -1 en la bandera indica que la validación es correcta.
    #                 validaciones_model.loc[23,'Observacion']="Error: Los poligonos deben numerarse en forma correlativa a partir del Nº '1'"
    #                 validaciones_model.loc[23,'Cetegoría']='Formularios'

    #             else:
    #                 validaciones_model.loc[23,'Resultado']=0 # si no hay un -1 en la bandera indica que la validación es correcta.
    #                 validaciones_model.loc[23,'Observacion']="OK: Numeros de poligonos en los bloques forms indicados correctamente"
    #                 validaciones_model.loc[23,'Cetegoría']='Formularios'
    #     else:

    #         validaciones_model.loc[23,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
    #         validaciones_model.loc[23,'Observacion']="OK: No corresponde validar los formularios dado que no se han insertado bloques para los mismos, verifique que se trate de un baldio"
    #         validaciones_model.loc[23,'Cetegoría']='Formularios'

            
    # else:
    #     validaciones_model.loc[21,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
    #     validaciones_model.loc[21,'Observacion']="OK: No corresponde validar los formularios dado que no se han insertado bloques para los mismos, verifique que se trate de un baldio"
    #     validaciones_model.loc[21,'Cetegoría']='Formularios'

    #     validaciones_model.loc[22,'Resultado']=99 # si no hay un -1 en la bandera indica que la validación es correcta.
    #     validaciones_model.loc[22,'Observacion']="OK: No corresponde validar los formularios dado que no se han insertado bloques para los mismos, verifique que se trate de un baldio"
    #     validaciones_model.loc[22,'Cetegoría']='Formularios'
    ...
    band_dict = {"band_mejora_cub_arc":band_mejora_cub_arc_0, "band_mejora_semi_arc":band_mejora_semi_arc_0, "band_mejora_desc_arc":band_mejora_desc_arc_0, "band_text_style":band_text_style,"band_parc_arc":band_parc_arc_1}

    mejoras_dict = {"piso_con_mejoras":piso_con_mejoras,"layers_mejoras_spb":layers_mejoras_spb,"sup_cub_dxf":sup_cub_dxf, "sup_semicub_dxf":sup_semicub_dxf, "sup_descub_dxf":sup_descub_dxf,"sup_descont_dxf": sup_descont_dxf}

    return validaciones_model, medidas_dxf, parcelas_poly_close, parcelas_poly, excedentes_poly,mejoras_dict, band_dict, nom_parc_list, manzana_poly, cesion_poly_close ,resultado_inv 
