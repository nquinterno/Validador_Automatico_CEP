    ...
    if len(pages_IFTAM_text)>0 and len(pages_IFFVN_text)>0 and len(pages_IFDOM_text)>0 and len(pages_FOMUBI_text)>0:
        ...
        if resultado_inv == "ok":
            ...
            if "invasion" in band_notas:
                ...
                validaciones_comparacion.loc[14,'Resultado']=-1
                validaciones_comparacion.loc[14,'Observacion']="Error: Se indicó nota de nivasion a via publica y no se represento construccion fuera de la manzana en el model"
            
            else:
                
                validaciones_comparacion.loc[14,'Resultado']=0
                validaciones_comparacion.loc[14,'Observacion']="Ok: No corresponde validar nota de invasion a via publica dado que no no se declararon construcciones fuera de la manzana"
            ...
            if "invasion" in band_notas:
                ...
                validaciones_comparacion.loc[14,'Resultado']=0
                validaciones_comparacion.loc[14,'Observacion']="Ok: Se indicó nota de ivasion a via publica y se represento construccion fuera de la manzana en el model"
            
            else:
                
                validaciones_comparacion.loc[14,'Resultado']=-1
                validaciones_comparacion.loc[14,'Observacion']="Error: Se indicó nota de ivasion a via publica y NO se represento construccion fuera de la manzana en el model"
